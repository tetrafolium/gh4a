...
public class ServiceFactory {
  ...
  private final static HttpLoggingInterceptor LOGGING_INTERCEPTOR =
      new HttpLoggingInterceptor().setLevel(HttpLoggingInterceptor.Level.BASIC);

  private final static Interceptor PAGINATION_INTRCEPTOR =
      new GitHubPaginationInterceptor();

  private final static Interceptor CACHE_STATUS_INTERCEPTOR = chain -> {
    Response response = chain.proceed(chain.request());
    Log.d("OkHttp",
          String.format(Locale.US, "For %s: network return code %d, cache %d",
                        response.request().url().toString(),
                        response.networkResponse() != null
                            ? response.networkResponse().code()
                            : -1,
                        response.cacheResponse() != null
                            ? response.cacheResponse().code()
                            : -1));
    return response;
  };
  ...
  // FIXME: The notifications endpoint currently returns invalid/empty ETags. GH
  // support
  //        says they're looking into it and to use If-Modified-Since in the
  //        meantime. Unfortunately, the Last-Modified header can't be relied on
  //        either, as it's not updated when marking notifications as read :-(
  //        We thus check for the invalid ETag and prevent caching if we found
  //        it. Once this is fixed on server side, this interceptor should be
  //        removed.
  ...
  private final static Interceptor CACHE_MAX_AGE_INTERCEPTOR = chain -> {
    ...
    // reload data afterwards. Make sure to constrain max age to 2 seconds to
    // only avoid network calls in cases where the exact same data is loaded
    // from multiple places at the same time, and use ETags to avoid useless
    // data transfers otherwise.
    if (origCacheControl.maxAgeSeconds() <= 2) {
      return response;
    }
    CacheControl.Builder newBuilder =
        new CacheControl.Builder().maxAge(2, TimeUnit.SECONDS);
  ...
  private final static Retrofit.Builder RETROFIT_BUILDER =
      new Retrofit.Builder()
          .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
          .addConverterFactory(new StringResponseConverterFactory())
          .addConverterFactory(
              MoshiConverterFactory.create(ServiceGenerator.moshi));
  ...
  public static <S> S get(final Class<S> serviceClass,
                          final boolean bypassCache) {
    return get(serviceClass, bypassCache, null, null, null);
  }

  public static <S> S get(final Class<S> serviceClass,
                          final boolean bypassCache, final String acceptHeader,
                          final String token, final Integer pageSize) {
    String key =
        makeKey(serviceClass, bypassCache, acceptHeader, token, pageSize);
    S service = (S)sCache.get(key);
    if (service == null) {
      service = createService(serviceClass, bypassCache, acceptHeader, token,
                              pageSize);
  ...
  private static String makeKey(final Class<?> serviceClass,
                                final boolean bypassCache,
                                final String acceptHeader, final String token,
                                final Integer pageSize) {
    return String.format(
        Locale.US, "%s-%d-%s-%s-%d", serviceClass.getSimpleName(),
        bypassCache ? 1 : 0, acceptHeader != null ? acceptHeader : "",
        token != null ? token : "", pageSize != null ? pageSize : 0);
  }

  private static <S> S createService(final Class<S> serviceClass,
                                     final boolean bypassCache,
                                     final String acceptHeader,
                                     final String token,
                                     final Integer pageSize) {
    OkHttpClient.Builder clientBuilder =
        sApiHttpClient.newBuilder()
            ...
            .addInterceptor(chain -> {
              ...
              Request.Builder requestBuilder = original.newBuilder().method(
                  original.method(), original.body());

              String tokenToUse =
                  token != null ? token : Gh4Application.get().getAuthToken();
              if (tokenToUse != null) {
    ...
    Retrofit retrofit = RETROFIT_BUILDER.baseUrl("https://api.github.com")
  ...
  createLoginService(final String userName, final String password,
  ...
  static void initClient(final Context context) {
    sApiHttpClient =
        enableTls12IfNeeded(new OkHttpClient.Builder())
            .cache(new Cache(new File(context.getCacheDir(), "api-http"),
                             20 * 1024 * 1024))
            .build();
    sImageHttpClient =
        new OkHttpClient.Builder()
            .cache(new Cache(new File(context.getCacheDir(), "image-http"),
                             20 * 1024 * 1024))
            .build();
  }

  private static OkHttpClient.Builder
  enableTls12IfNeeded(final OkHttpClient.Builder builder) {
  ...
  public interface LoginService {
    ...
    @POST("/authorizations")
    Single<retrofit2.Response<AuthorizationResponse>>
    createAuthorization(@Body AuthorizationRequest request);
    ...
    class AuthorizationRequest {
      ...
      public AuthorizationRequest(final String scopes, final String note,
