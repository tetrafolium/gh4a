...
public class Gh4Application
  ...
  public void onCreate() {
    ...
    if (prefsVersion < 3) {
      SharedPreferences.Editor editor = prefs.edit().putInt(KEY_VERSION, 3);
  ...
  private void updateNotificationJob(final SharedPreferences prefs) {
    if (isAuthorized() &&
        prefs.getBoolean(SettingsFragment.KEY_NOTIFICATIONS, false)) {
      int intervalMinutes =
          prefs.getInt(SettingsFragment.KEY_NOTIFICATION_INTERVAL, 15);
  ...
  public void setActiveLogin(final String login) {
    if (getPrefs().getStringSet(KEY_ALL_LOGINS, null).contains(login)) {
      getPrefs().edit().putString(KEY_ACTIVE_LOGIN, login).apply();
    }
  }
  ...
  public String getAuthToken() {
    ...
    return login != null ? getPrefs().getString(KEY_PREFIX_TOKEN + login, null)
                         : null;
  }
  ...
  public void logout() {
    ...
    getPrefs()
        .edit()
        .putString(KEY_ACTIVE_LOGIN,
                   logins.size() > 0 ? logins.iterator().next() : null)
        .putStringSet(KEY_ALL_LOGINS, logins)
  ...
  public static Gh4Application get() { return sInstance; }
