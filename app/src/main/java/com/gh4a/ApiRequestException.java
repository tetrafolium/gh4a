...
public class ApiRequestException extends RuntimeException {
  ...
  public ApiRequestException(final Response response) {
    ...
    try {
      error = ServiceGenerator.moshi.adapter(ClientErrorResponse.class)
  ...
  public ClientErrorResponse getResponse() { return mResponse; }
  ...
  public String getMessage() {
    ...
    if (!TextUtils.isEmpty(message) && !errors.isEmpty()) {
      return String.format(Locale.US, "%1$s (%2$d) [%3$s]", message, mStatus,
  ...
  private String formatFieldError(final ClientErrorResponse.FieldError error) {
    ...
    case MissingField:
      return String.format("Value for required field %1$s is missing",
                           error.field());
