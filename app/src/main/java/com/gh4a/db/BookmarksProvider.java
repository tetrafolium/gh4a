...
public class BookmarksProvider extends ContentProvider {
  ...
  public interface Columns extends BaseColumns {
    Uri CONTENT_URI = Uri.parse(String.format(
        Locale.US, "content://%s/bookmarks", BuildConfig.APPLICATION_ID));
  ...
  public static void saveBookmark(final Context context, final String name,
                                  final int type, final String url,
    ...
    if (cr.insert(BookmarksProvider.Columns.CONTENT_URI, cv) != null &&
        showToast) {
      Toast.makeText(context, R.string.bookmark_saved, Toast.LENGTH_LONG)
          .show();
    }
  }

  private static int getNextOrderId(final ContentResolver cr) {
    Cursor query = cr.query(Columns.CONTENT_URI, new String[] {"COUNT(*)"},
                            null, null, null);
  ...
  public static void removeBookmark(final Context context, final String url) {
    int removedRows = context.getContentResolver().delete(
        Columns.CONTENT_URI, Columns.URI + " = ?", new String[] {url});
    if (removedRows > 0) {
      Toast.makeText(context, R.string.bookmark_removed, Toast.LENGTH_SHORT)
          .show();
    }
  }
  ...
  public static boolean hasBookmarked(final Context context, final String url) {
    Cursor cursor = context.getContentResolver().query(
        Columns.CONTENT_URI, new String[] {Columns._ID}, Columns.URI + " = ?",
        new String[] {url}, null);
  ...
  public static void reorderBookmark(final Context context, final long id,
                                     final int orderId) {
    ContentValues cv = new ContentValues();
  ...
  public int update(final @NonNull Uri uri, final ContentValues values,
    ...
    case MATCH_ALL:
      count =
          db.update(DbHelper.BOOKMARKS_TABLE, values, selection, selectionArgs);
      break;
    case MATCH_ID:
