...
public class DbHelper extends SQLiteOpenHelper {
  ...
  private void createBookmarksTable(final SQLiteDatabase db,
  ...
  private void updateBookmarkUris(final SQLiteDatabase db) {
    Cursor c =
        db.query(BOOKMARKS_TABLE, new String[] {"_id", "uri", "extra_data"},
    ...
    try {
      ...
      while (c.moveToNext()) {
        ...
        try {
          ...
          } else if ("com.gh4a.activities.RepositoryActivity".equals(
                         activity)) {
          ...
          if (url != null) {
            cv.put("uri", url);
            db.update(BOOKMARKS_TABLE, cv, "_id = ?",
  ...
  private void addBookmarksOrderIdColumn(final SQLiteDatabase db) {
    ...
    db.execSQL(
        "INSERT INTO temp_bookmarks (_id, name, type, uri, extra_data, order_id) "
        + "SELECT _id, name, type, uri, extra_data, "
        + "(SELECT COUNT(*) - 1 FROM " + BOOKMARKS_TABLE +
        " b WHERE a._id >= b._id) "
