...
public abstract class BasePagerActivity
  ...
  protected void adjustTabsForHeaderAlignedFab(final boolean fabPresent) {
    int margin = fabPresent ? getResources().getDimensionPixelSize(
                                  R.dimen.mini_fab_size_with_margin)
                            : 0;

    ViewGroup.MarginLayoutParams lp =
        (ViewGroup.MarginLayoutParams)mTabs.getLayoutParams();
  ...
  protected ViewPager getPager() { return mPager; }
  ...
  private ViewPager setupPager() {
    ...
    if (hasTabsInToolbar()) {
      ...
      toolbar.addView(
          mTabs, new Toolbar.LayoutParams(Toolbar.LayoutParams.WRAP_CONTENT,
  ...
  private void updateTabVisibility() {
    ...
    mTabs.setVisibility(count > 1 && !mErrorViewVisible ? View.VISIBLE
                                                        : View.GONE);
  ...
  protected int[][] getTabHeaderColorAttrs() { return null; }

  /* expected format: int[2] - 0 is header, 1 is status bar */
  protected int[] getHeaderColorAttrs() { return null; }
  ...
  protected void onPageMoved(final int position, final float fraction) {
    if (mTabHeaderColors != null) {
      int nextIndex =
          Math.max(0, Math.min(position + 1, mTabHeaderColors.length - 1));
      int headerColor =
          UiUtils.mixColors(mTabHeaderColors[position][0],
                            mTabHeaderColors[nextIndex][0], fraction);
      int statusBarColor =
          UiUtils.mixColors(mTabHeaderColors[position][1],
