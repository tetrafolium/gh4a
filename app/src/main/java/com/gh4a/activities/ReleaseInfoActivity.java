...
public class ReleaseInfoActivity extends BaseActivity
    implements View.OnClickListener, SwipeRefreshLayout.ChildScrollDelegate,
  ...
  public void onItemClick(final ReleaseAsset item) {
    UiUtils.enqueueDownloadWithPermissionCheck(
        this, item.url(), item.contentType(), item.name(), item.label(),
  ...
  public void onClick(final View v) {
    ...
    case R.id.tv_releasetag:
      intent = RepositoryActivity.makeIntent(this, mRepoOwner, mRepoName,
                                             mRelease.tagName());
      break;
  ...
  private void loadRelease(final boolean force) {
    RepositoryReleaseService service =
        ServiceFactory.get(RepositoryReleaseService.class, force);
  ...
  private void loadBody() {
    ...
    if (TextUtils.isEmpty(mRelease.body())) {
      ...
      MarkdownService service =
          ServiceFactory.get(MarkdownService.class, false);
    ...
    mBodySubscription =
        htmlSingle.compose(makeLoaderSingle(ID_LOADER_BODY, false))
