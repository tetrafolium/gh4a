...
public class RepositoryActivity extends BaseFragmentPagerActivity
    implements CommitListFragment.ContextSelectionCallback,
  ...
  public static Intent makeIntent(final Context context, final String repoOwner,
    ...
    return makeIntent(context, repoOwner, repoName, ref, null,
                      PAGE_REPO_OVERVIEW);
  }
  ...
  private static final int[] TITLES =
      new int[] {R.string.about, R.string.repo_files, R.string.commits,
                 R.string.repo_activity};
  ...
  protected Fragment makeFragment(final int position) {
    ...
    case 1:
      Fragment f = ContentListContainerFragment.newInstance(
          mRepository, mSelectedRef, mInitialPath);
      mInitialPath = null;
      return f;
    case 2:
  ...
  protected boolean fragmentNeedsRefresh(final Fragment fragment) {
    if (fragment instanceof CommitListFragment && mCommitListFragment == null) {
      ...
      return true;
               ...
               mRepositoryFragment == null) {
      return true;
    } else if (fragment instanceof RepositoryEventListFragment &&
               mActivityFragment == null) {
      return true;
    }
    return false;
  ...
  public boolean onPrepareOptionsMenu(final Menu menu) {
    if (mRepository == null) {
      ...
      if (bookmarkAction != null) {
        bookmarkAction.setTitle(
            BookmarksProvider.hasBookmarked(this, getBookmarkUrl())
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    case R.id.search:
      ...
      startActivity(SearchActivity.makeIntent(
          this, initialSearch, SearchActivity.SEARCH_TYPE_CODE, false));
      return true;
    case R.id.bookmark:
      ...
      if (BookmarksProvider.hasBookmarked(this, bookmarkUrl)) {
        ...
        BookmarksProvider.saveBookmark(this, mActionBar.getTitle().toString(),
                                       BookmarksProvider.Columns.TYPE_REPO,
  ...
  private void showRefSelectionDialog() {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (item.name().equals(mSelectedRef) ||
          item.commit().sha().equals(mSelectedRef)) {
        current = i;
      }
    ...
    new AlertDialog.Builder(this)
        ...
        .setSingleChoiceItems(adapter, current,
                              (dialog, which) -> {
  ...
  private void loadRepository(final boolean force) {
    RepositoryService service =
        ServiceFactory.get(RepositoryService.class, force);
    service.getRepository(mRepoOwner, mRepoName)
        ...
        .subscribe(result -> {
          ...
          if (mInitialPage >= PAGE_REPO_OVERVIEW &&
              mInitialPage <= PAGE_ACTIVITY) {
  ...
  private void loadOrShowRefSelection() {
    if (mBranches != null) {
      ...
      final RepositoryService repoService =
          ServiceFactory.get(RepositoryService.class, false);

      Single<List<Branch>> branchSingle = ApiHelpers.PageIterator.toSingle(
          page -> branchService.getBranches(mRepoOwner, mRepoName, page));
      Single<List<Branch>> tagSingle = ApiHelpers.PageIterator.toSingle(
          page -> repoService.getTags(mRepoOwner, mRepoName, page));

      registerTemporarySubscription(
          Single.zip(branchSingle, tagSingle, Pair::create)
  ...
  private class BranchAndTagAdapter extends BaseAdapter {
    ...
    public BranchAndTagAdapter() {
      ...
      mBranchDrawableResId =
          UiUtils.resolveDrawable(RepositoryActivity.this, R.attr.branchIcon);
      mTagDrawableResId =
          UiUtils.resolveDrawable(RepositoryActivity.this, R.attr.tagIcon);
    }
    ...
    public View getView(final int position, final View convertView,
      ...
      icon.setImageResource(position >= mFirstTagIndex ? mTagDrawableResId
                                                       : mBranchDrawableResId);
