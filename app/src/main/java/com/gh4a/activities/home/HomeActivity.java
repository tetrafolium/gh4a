...
public class HomeActivity extends BaseFragmentPagerActivity
    implements View.OnClickListener, RepositoryListContainerFragment.Callback,
               NotificationListFragment.ParentCallback,
               UserPasswordLoginDialogFragment.ParentCallback {
  public static Intent makeIntent(final Context context,
                                  final @IdRes int initialPageId) {
  ...
  public static Intent makeNotificationsIntent(final Context context,
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    mNotificationsIndicatorIcon = DrawableCompat.wrap(
  ...
  private void updateNotificationIndicator(final int checkedItemId) {
    ...
    @AttrRes
    int colorResId =
        checkedItemId == R.id.notifications ? R.attr.colorAccent
  ...
  public boolean onNavigationItemSelected(final @NonNull MenuItem item) {
    ...
    case R.id.add_account:
      new LoginModeChooserFragment().show(getSupportFragmentManager(),
                                          "loginmode");
      return true;
    case R.id.settings:
    ...
    if (id >= OTHER_ACCOUNTS_GROUP_BASE_ID &&
        id < OTHER_ACCOUNTS_GROUP_BASE_ID + accountCount) {
  ...
  protected void onActivityResult(final int requestCode, final int resultCode,
    ...
    if (requestCode == REQUEST_SETTINGS) {
      if (data.getBooleanExtra(SettingsActivity.RESULT_EXTRA_THEME_CHANGED,
                               false)) {
        goToToplevelActivity();
        finish();
      }
  ...
  public void supportInvalidateOptionsMenu() {
    ...
    if (mFactory instanceof RepositoryFactory) {
      // happens when load is done; we ignore it as we don't want to close the
      // IME in that case
    } else {
  ...
  public void toggleToolDrawer() { toggleRightSideDrawer(); }
  ...
  private int determineInitialPage() {
    String initialPage =
        getIntent().hasExtra("initial_page")
  ...
  private void updateDrawerMode(final boolean accountMode) {
    ...
    if (accountMode) {
      ...
      for (int i = 0;; i++) {
        MenuItem item =
            mLeftDrawerMenu.findItem(OTHER_ACCOUNTS_GROUP_BASE_ID + i);
        if (item == null) {
          break;
        }
    ...
    mDrawerSwitcher.setImageResource(accountMode
                                         ...
                                         : R.drawable.drop_down_arrow_white);
  ...
  private void switchTo(final int itemId, final FragmentFactory factory) {
    ...
    getPrefs()
        .edit()
        .putString("last_selected_home_page",
                   START_PAGE_MAPPING.get(mSelectedFactoryId))
        .apply();
  ...
  private void loadNotificationIndicator(final boolean force) {
    NotificationService service =
        ServiceFactory.get(NotificationService.class, force, null, null, 1);
    ...
    service.getNotifications(options, 1)
        ...
        .subscribe(this::setNotificationsIndicatorVisible,
                   this::handleLoadFailure);
  }

  public static class LoginModeChooserFragment
    ...
    public void onClick(final DialogInterface dialog, final int which) {
      if (which == DialogInterface.BUTTON_POSITIVE) {
        new BrowserLogoutDialogFragment().show(getFragmentManager(),
                                               "browserlogout");
      } else {
        UserPasswordLoginDialogFragment
            .newInstance(Github4AndroidActivity.SCOPES)
            .show(getFragmentManager(), "login");
      }
    }
  }

  public static class BrowserLogoutDialogFragment
      extends DialogFragment implements DialogInterface.OnClickListener {
    @NonNull
    @Override
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
    ...
    public void onClick(final DialogInterface dialog, final int which) {
      if (which == DialogInterface.BUTTON_NEUTRAL) {
        ...
        new BrowserLogoutCompletedDialogFragment().show(
            fm, "browserlogoutcomplete");
      }
    }
  }

  public static class BrowserLogoutCompletedDialogFragment
      extends DialogFragment implements DialogInterface.OnClickListener {
    @NonNull
    @Override
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
