...
public class CreateReviewActivity
  ...
  protected static Intent makeIntent(final Context context,
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    View header =
        getLayoutInflater().inflate(R.layout.create_review_header, null);
    mEditorSheet.addHeaderView(header);

    mReviewEventAdapter = new ArrayAdapter<>(this, R.layout.spinner_item);
    mReviewEventAdapter.add(new ReviewEventDesc(
        R.string.pull_request_review_event_comment, CreateReview.Event.Comment,
        SubmitReview.Event.Comment));
    mReviewEventAdapter.add(new ReviewEventDesc(
        R.string.pull_request_review_event_approve, CreateReview.Event.Approve,
        SubmitReview.Event.Approve));
    mReviewEventAdapter.add(new ReviewEventDesc(
        R.string.pull_request_review_event_request_changes,
    ...
    titleView.setText(getString(R.string.pull_request_review_dialog_title,
                                mPullRequestNumber));
  ...
  public Single<?> onEditorDoSend(final String body) {
    ...
    if (mPendingReview == null) {
      CreateReview request =
          CreateReview.builder().body(body).event(desc.mCreateEvent).build();
      resultSingle = service.createReview(mRepoOwner, mRepoName,
                                          mPullRequestNumber, request);
    } else {
      SubmitReview request =
          SubmitReview.builder().body(body).event(desc.mSubmitEvent).build();
      resultSingle =
          service.submitReview(mRepoOwner, mRepoName, mPullRequestNumber,
                               mPendingReview.id(), request);
    }
  ...
  private class ReviewEventDesc {
    @StringRes public final int mTextResId;
    public final CreateReview.Event mCreateEvent;
    public final SubmitReview.Event mSubmitEvent;

    public ReviewEventDesc(final @StringRes int textResId,
                           final CreateReview.Event createEvent,
