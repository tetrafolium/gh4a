...
public abstract class DiffViewerActivity<C extends PositionalCommentBase>
    extends WebViewerActivity
    implements ReactionBar.Callback, ReactionBar.ReactionDetailsCache.Listener {
  protected static <C extends PositionalCommentBase> Intent
  fillInIntent(final Intent baseIntent, final String repoOwner,
               final String repoName, final String commitSha, final String path,
               final String diff, final List<C> comments, final int initialLine,
    ...
    return baseIntent.putExtra("owner", repoOwner)
        ...
        .putExtra("diff", diff)
        .putExtra("comments",
                  comments != null ? new ArrayList<>(comments) : null)
  ...
  private static final String REACTION_PLUS_ONE_PATH =
      "M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2"
      +
  ...
  private static final String REACTION_MINUS_ONE_PATH =
      "M19,15H23V3H19M15,3H6"
      +
  ...
  private static final String REACTION_CONFUSED_PATH =
      "M8.5,11A1.5,1.5 0 0,1 7,9.5"
  ...
  private static final String REACTION_HEART_PATH =
      "M12,21.35L10.55,20.03"
      ...
      + "L12,21.35Z";
  private static final String REACTION_HOORAY_PATH =
      "M11.5,0.5C12,0.75 13,2.4 13,3.5"
      +
  ...
  private static final String REACTION_LAUGH_PATH =
      "M12,17.5C14.33,17.5 16.3,16.04 17.11,14"
      +
  ...
  private final SparseArray<List<PositionalCommentBase>> mCommitCommentsByPos =
      new SparseArray<>();
  private final LongSparseArray<CommitCommentWrapper> mCommitComments =
      new LongSparseArray<>();
  ...
  public boolean onCreateOptionsMenu(final Menu menu) {
    ...
    String viewAtTitle =
        getString(R.string.object_view_file_at, mSha.substring(0, 7));
  ...
  protected String generateHtml(final String cssTheme,
    ...
    for (int i = 0; i < mDiffLines.length; i++) {
      ...
      if (authorized) {
        String uri = String.format(Locale.US, COMMENT_ADD_URI_FORMAT, i,
                                   leftDiffPosition, rightDiffPosition,
                                   line.startsWith("+"));
      ...
      if (comments != null) {
        for (PositionalCommentBase comment : comments) {
          ...
          content.append("\"");
          if (authorized) {
            String uri = String.format(Locale.US, COMMENT_EDIT_URI_FORMAT, i,
                                       leftDiffPosition, rightDiffPosition,
                                       line.startsWith("+"), id);
            content.append(" onclick=\"javascript:location.href='");
            content.append(uri).append("'\"");
          ...
          content.append(getString(
              R.string.commit_comment_header,
          ...
          if (reactions.totalCount() > 0) {
            content.append("<div>");
            appendReactionSpan(content, reactions.plusOne(),
                               REACTION_PLUS_ONE_PATH);
            appendReactionSpan(content, reactions.minusOne(),
                               REACTION_MINUS_ONE_PATH);
            appendReactionSpan(content, reactions.confused(),
                               REACTION_CONFUSED_PATH);
            ...
            appendReactionSpan(content, reactions.hooray(),
                               REACTION_HOORAY_PATH);
            content.append("</div>");
          }
          content.append("</div>");
        }
    ...
    if (mInitialLine > 0) {
      content.insert(highlightInsertPos,
                     " onload='scrollToElement(\"line" + mInitialLine +
                         "\")' onresize='scrollToHighlight();'");
    } else if (mInitialComment != null) {
      content.insert(highlightInsertPos,
                     " onload='scrollToElement(\"comment" +
                         mInitialComment.commentId +
                         "\")' onresize='scrollToHighlight();'");
    } else if (highlightStartLine != -1 && highlightEndLine != -1) {
      content.insert(highlightInsertPos,
                     " onload='highlightDiffLines(" + highlightStartLine + "," +
                         highlightEndLine +
                         ")' onresize='scrollToHighlight();'");
    }
  ...
  private void appendReactionSpan(final StringBuilder content, final int count,
                                  final String iconPathContents) {
    if (count == 0) {
      return;
    }
    content.append("<span class='reaction'>");
    content.append(
        "<svg width='14px' height='14px' viewBox='0 0 24 24'><path d='");
  ...
  protected String getDocumentTitle() {
    return getString(R.string.diff_print_document_title,
                     FileUtils.getFileName(mPath), mSha.substring(0, 7),
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    case R.id.share:
      ...
      return true;
    case MENU_ITEM_VIEW:
      startActivity(FileViewerActivity.makeIntent(this, mRepoOwner, mRepoName,
                                                  mSha, mPath));
      return true;
    }
  ...
  private void addCommentsToMap(final List<C> comments) {
    ...
    for (PositionalCommentBase comment : comments) {
      ...
      List<PositionalCommentBase> commentsByPos =
          mCommitCommentsByPos.get(position);
  ...
  protected void handleUrlLoad(final Uri uri) {
    ...
    boolean isRightLine =
        Boolean.parseBoolean(uri.getQueryParameter("isRightLine"));
    String lineText = mDiffLines[line];
    String idParam = uri.getQueryParameter("id");
    long id = idParam != null ? Long.parseLong(idParam) : 0L;

    CommentActionPopup p =
        new CommentActionPopup(id, line, lineText, leftLine, rightLine,
                               mLastTouchDown.x, mLastTouchDown.y, isRightLine);
    p.show();
  }

  private void refresh() {
    // Make sure we load the comments from remote, as we now know they've
    // changed
  ...
  openCommentDialog(long id, long replyToId, String line, int position,
                    int leftLine, int rightLine,
                    PositionalCommentBase commitComment);
  ...
  protected abstract PositionalCommentBase
  onUpdateReactions(PositionalCommentBase comment, Reactions reactions);
  ...
  private void loadComments(final boolean useIntentExtraIfPresent,
                            final boolean force) {
    List<C> intentComments =
        useIntentExtraIfPresent
            ? getIntent().getParcelableArrayListExtra("comments")
            : null;
    Single<List<C>> commentSingle =
        intentComments != null
            ? Single.just(intentComments)
            : createCommentSingle(force).compose(
                  makeLoaderSingle(ID_LOADER_COMMENTS, force));
  ...
  private class CommentActionPopup
    ...
    public CommentActionPopup(final long id, final int position,
                              final String lineText, final int leftLine,
                              final int rightLine, final int x, final int y,
      ...
      if (id == 0 ||
          !ApiHelpers.loginEquals(comment.comment.user(), ownLogin)) {
      ...
      if (id != 0) {
        ...
        mReactionMenuHelper = new ReactionBar.AddReactionMenuHelper(
            DiffViewerActivity.this, reactionMenu, DiffViewerActivity.this,
            comment, mReactionDetailsCache);
    ...
    public boolean onMenuItemClick(final MenuItem item) {
      if (mReactionMenuHelper != null &&
          mReactionMenuHelper.onItemClick(item)) {
        return true;
      }
      ...
      case R.id.delete:
        new AlertDialog.Builder(DiffViewerActivity.this)
            ...
            .setPositiveButton(R.string.delete,
                               (dialog, which) -> deleteComment(mId))
      ...
      case R.id.edit:
        ...
        PositionalCommentBase comment =
            wrapper != null ? wrapper.comment : null;
      ...
      case R.id.share:
        Uri url =
            createUrl(createLineLinkId(mIsRightLine ? mRightLine : mLeftLine,
                                       mIsRightLine),
                      mId);
        String subject =
