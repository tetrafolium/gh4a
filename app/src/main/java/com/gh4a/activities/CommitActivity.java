...
public class CommitActivity extends BaseFragmentPagerActivity
    implements CommitFragment.CommentUpdateListener,
               CommitNoteFragment.CommentUpdateListener {
  public static Intent makeIntent(final Context context, final String repoOwner,
                                  final String repoName, final String sha) {
  ...
  public static Intent makeIntent(final Context context, final String repoOwner,
                                  ...
                                  final String sha) {
    return makeIntent(context, repoOwner, repoName, pullRequestNumber, sha,
                      null);
  }
  ...
  private static final int[] TITLES =
      new int[] {R.string.commit, R.string.issue_comments};
  ...
  protected Fragment makeFragment(final int position) {
    if (position == 1) {
      Fragment f =
          CommitNoteFragment.newInstance(mRepoOwner, mRepoName, mObjectSha,
  ...
  private void loadCommit(final boolean force) {
    RepositoryCommitService service =
        ServiceFactory.get(RepositoryCommitService.class, force);
