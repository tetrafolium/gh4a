...
public class GistViewerActivity extends WebViewerActivity {
  public static Intent makeIntent(final Context context, final String id,
  ...
  protected String generateHtml(final String cssTheme,
    ...
    if (FileUtils.isMarkdown(mGistFile.filename())) {
      ...
      return generateMarkdownHtml(base64Data, null, null, null, cssTheme,
                                  addTitleHeader);
    } else {
      return generateCodeHtml(mGistFile.content(), mFileName, -1, -1, cssTheme,
                              addTitleHeader);
    }
  }
  ...
  protected String getDocumentTitle() {
    return getString(R.string.gist_print_document_title, mFileName, mGistId,
                     mGistOwner);
  }
  ...
  private void loadGist(final boolean force) {
    ...
    service.getGist(mGistId)
        ...
        .subscribe(result -> {
          mGistOwner =
              ApiHelpers.getUserLogin(GistViewerActivity.this, result.owner());
