...
public class IssueListActivity extends BaseFragmentPagerActivity
  ...
  private static final String STATE_KEY_SEARCH_IS_EXPANDED =
      "search_is_expanded";
  private static final String STATE_KEY_SELECTED_MILESTONE =
      "selected_milestone";
  ...
  private static final String STATE_KEY_PARTICIPATING_STATUS =
  ...
  private static final int[] PULL_REQUEST_TITLES =
      new int[] {R.string.open, R.string.closed, R.string.merged};
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      mSearchIsExpanded =
          savedInstanceState.getBoolean(STATE_KEY_SEARCH_IS_EXPANDED);
      mSelectedLabel = savedInstanceState.getString(STATE_KEY_SELECTED_LABEL);
      mSelectedAssignee =
          savedInstanceState.getString(STATE_KEY_SELECTED_ASSIGNEE);
  ...
  protected void onSaveInstanceState(final Bundle outState) {
    ...
    outState.putInt(STATE_KEY_PARTICIPATING_STATUS,
                    mSelectedParticipatingStatus);
  }
  ...
  protected Fragment makeFragment(final int position) {
    ...
    if (mSearchMode) {
      query = String.format(
          Locale.US, SEARCH_QUERY, mIsPullRequest ? "pr" : "issue",
          getIssueType(position), mRepoOwner, mRepoName, mSearchQuery);
      emptyTextResId = mIsPullRequest ? R.string.no_search_pull_requests_found
                                      : R.string.no_search_issues_found;
    } else {
      query =
          String
              .format(Locale.US, LIST_QUERY, mIsPullRequest ? "pr" : "issue",
              ...
              .trim();
      emptyTextResId = mIsPullRequest ? R.string.no_pull_requests_found
                                      : R.string.no_issues_found;
    }

    return IssueListFragment.newInstance(
        query, mSortHelper.getSortMode(), mSortHelper.getSortOrder(),
  ...
  public void onActivityResult(final int requestCode, final int resultCode,
    ...
    if (requestCode == REQUEST_ISSUE_CREATE) {
      ...
      // Refresh all fragments (instead of just open issues fragment) when
      // coming back from newly created issue as there is a chance that it could
  ...
  protected int[] getRightNavigationDrawerMenuResources() {
    ...
    if (!mSearchMode) {
      menuResIds[1] = mIsCollaborator != null && mIsCollaborator
                          ? R.menu.issue_list_filter_collab
                          : R.menu.issue_list_filter;
    }
    return menuResIds;
  }
  ...
  protected void onPrepareRightNavigationDrawerMenu(final Menu menu) {
    ...
    if (milestoneFilterItem != null) {
      final String subtitle =
          mSelectedMilestone == null
              ? getString(R.string.issue_filter_by_any_milestone)
              : mSelectedMilestone.isEmpty()
                    ? getString(R.string.issue_filter_by_no_milestone)
                    : mSelectedMilestone;
      UiUtils.setMenuItemText(this, milestoneFilterItem,
                              getString(R.string.issue_filter_by_milestone),
                              subtitle);
    }
    ...
    if (labelFilterItem != null) {
      final String subtitle =
          mSelectedLabel == null
              ? getString(R.string.issue_filter_by_any_label)
              : mSelectedLabel.isEmpty()
                    ? getString(R.string.issue_filter_by_no_label)
                    : mSelectedLabel;
      UiUtils.setMenuItemText(this, labelFilterItem,
                              getString(R.string.issue_filter_by_labels),
                              subtitle);
    }
    ...
    if (assigneeFilterItem != null) {
      final String subtitle =
          mSelectedAssignee == null
              ? getString(R.string.issue_filter_by_any_assignee)
              : mSelectedAssignee.isEmpty()
                    ? getString(R.string.issue_filter_by_no_assignee)
                    : mSelectedAssignee;
      UiUtils.setMenuItemText(this, assigneeFilterItem,
                              getString(R.string.issue_filter_by_assignee),
                              subtitle);
    }
    MenuItem participatingFilterItem =
        menu.findItem(R.id.filter_by_participating);
    if (participatingFilterItem != null) {
      String[] valueStrings =
          getResources().getStringArray(R.array.filter_participating);
  ...
  public boolean onNavigationItemSelected(final @NonNull MenuItem item) {
    ...
    case R.id.manage_labels:
      startActivity(IssueLabelListActivity.makeIntent(
          this, mRepoOwner, mRepoName, mIsPullRequest));
      return true;
    case R.id.manage_milestones:
      startActivity(IssueMilestoneListActivity.makeIntent(
          this, mRepoOwner, mRepoName, mIsPullRequest));
      return true;
    }
  ...
  public void onClick(final View view) {
    Intent intent =
        IssueEditActivity.makeCreateIntent(this, mRepoOwner, mRepoName);
  ...
  private void showMilestonesDialog() {
    ...
    int selected =
        mSelectedMilestone != null && mSelectedMilestone.isEmpty() ? 1 : 0;

    milestones[0] =
        getResources().getString(R.string.issue_filter_by_any_milestone);
    milestones[1] =
        getResources().getString(R.string.issue_filter_by_no_milestone);
  ...
  private void showAssigneesDialog() {
    ...
    int selected =
        mSelectedAssignee != null && mSelectedAssignee.isEmpty() ? 1 : 0;

    assignees[0] =
        getResources().getString(R.string.issue_filter_by_any_assignee);
    assignees[1] =
        getResources().getString(R.string.issue_filter_by_no_assignee);
    ...
    DialogInterface.OnClickListener selectCb = (dialog, which) -> {
      mSelectedAssignee =
          which == 0 ? null
                     : which == 1 ? "" : mAssignees.get(which - 2).login();
      dialog.dismiss();
      onFilterUpdated();
    };
  ...
  private void filterAssignee() {
    if (mAssignees != null) {
      ...
      final IssueAssigneeService service =
          ServiceFactory.get(IssueAssigneeService.class, false);
      registerTemporarySubscription(
          ApiHelpers.PageIterator
              .toSingle(
                  page -> service.getAssignees(mRepoOwner, mRepoName, page))
              .compose(RxUtils::doInBackground)
              .compose(
                  RxUtils.wrapWithProgressDialog(this, R.string.loading_msg))
              .subscribe(assignees -> {
  ...
  private void filterMilestone() {
    if (mMilestones != null) {
      ...
      registerTemporarySubscription(
          ApiHelpers.PageIterator
              .toSingle(page
                        -> service.getRepositoryMilestones(
                            mRepoOwner, mRepoName, "open", page))
              .compose(RxUtils::doInBackground)
              .compose(
                  RxUtils.wrapWithProgressDialog(this, R.string.loading_msg))
    ...
    if (mLabels != null) {
      ...
      registerTemporarySubscription(
          ApiHelpers.PageIterator
              .toSingle(
                  page
                  -> service.getRepositoryLabels(mRepoOwner, mRepoName, page))
              .compose(RxUtils::doInBackground)
              .compose(
                  RxUtils.wrapWithProgressDialog(this, R.string.loading_msg))
              .subscribe(labels -> {
  ...
  private void filterParticipating() {
    ...
    new AlertDialog.Builder(this)
        ...
        .setSingleChoiceItems(R.array.filter_participating,
                              mSelectedParticipatingStatus, selectCb)
        .setNegativeButton(R.string.cancel, null)
        .show();
  }
