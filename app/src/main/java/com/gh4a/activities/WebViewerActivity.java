...
public abstract class WebViewerActivity extends BaseActivity
    implements SwipeRefreshLayout.ChildScrollDelegate, View.OnTouchListener {
  ...
  private final int[] ZOOM_SIZES = new int[] {50, 75, 100, 150, 200};
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    setContentView(
        LayoutInflater.from(inflateContext).inflate(R.layout.web_viewer, null));
  ...
  private void setupWebView() {
    ...
    mWebView.setBackgroundColor(
        UiUtils.resolveColor(this, R.attr.colorWebViewBackground));
  ...
  private void doPrint() {
    ...
    if (mRequiresJsInterface) {
      mPrintWebView.addJavascriptInterface(new PrintJavascriptInterface(),
                                           "NativeClient");
    } else {
    ...
    mPrintWebView.loadDataWithBaseURL("file:///android_asset/", html, null,
                                      "utf-8", null);
  ...
  private void doPrintHtml() {
    if (!isFinishing()) {
      ...
      PrintDocumentAdapter printAdapter =
          getPrintAdapterForWebView(mPrintWebView, title);
  ...
  @SuppressWarnings("deprecation")
  @TargetApi(19)
  private PrintDocumentAdapter getPrintAdapterForWebView(final WebView webView,
                                                         final String title) {
  ...
  protected void onDataReady() {
    ...
    if (mRequiresJsInterface) {
      mWebView.addJavascriptInterface(new DisplayJavascriptInterface(),
                                      "NativeClient");
    }
    mWebView.loadDataWithBaseURL("file:///android_asset/", html, null, "utf-8",
                                 null);
    mHasData = true;
    supportInvalidateOptionsMenu();
  }
  ...
  protected String generateMarkdownHtml(final String base64Data,
    ...
    if (repoOwner != null && repoName != null) {
      String urlPrefix = "https://raw.github.com/" + repoOwner + "/" +
                         repoName + "/" + (ref != null ? ref : "master");
  ...
  protected static String wrapUnthemedHtml(final String html,
                                           final String cssTheme,
                                           final String title) {
  ...
  protected static void writeScriptInclude(final StringBuilder builder,
  ...
  protected static void writeCssInclude(final StringBuilder builder,
  ...
  @Override protected abstract boolean canSwipeToRefresh();

  protected boolean handlePrintRequest() { return false; }
  protected abstract String generateHtml(String cssTheme,
                                         boolean addTitleHeader);
  ...
  private class DisplayJavascriptInterface {
    ...
    public void onLineTouched(final int line) {
      mHandler.post(()
                        -> WebViewerActivity.this.onLineTouched(
                            line, mLastTouchDown.x, mLastTouchDown.y));
    }
