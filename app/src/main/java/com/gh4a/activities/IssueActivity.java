...
public class IssueActivity
  ...
  makeIntent(final Context context, final String login, final String repoName,
             final int number,
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    setFragment((IssueFragment)getSupportFragmentManager().findFragmentById(
        R.id.details));
  ...
  private void showUiIfDone() {
    ...
    IssueFragment newFragment = IssueFragment.newInstance(
        mRepoOwner, mRepoName, mIssue, mIsCollaborator, mInitialComment);
    if (mFragment != null) {
  ...
  private void updateHeader() {
    ...
    transitionHeaderToColor(stateColorAttributeId,
                            closed ? R.attr.colorIssueClosedDark
                                   : R.attr.colorIssueOpenDark);
  ...
  public boolean onCreateOptionsMenu(final Menu menu) {
    ...
    boolean isCreator = mIssue != null && authorized &&
                        ApiHelpers.loginEquals(
                            mIssue.user(), Gh4Application.get().getAuthLogin());
    ...
    boolean canClose =
        mIssue != null && authorized && (isCreator || isCollaborator);
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    case R.id.share:
      IntentUtils.share(this,
                        getString(R.string.share_issue_subject, mIssueNumber,
                                  mIssue.title(), mRepoOwner + "/" + mRepoName),
  ...
  public void onRefresh() {
    ...
    // onRefresh() can be triggered in the draw loop, and CoordinatorLayout
    // doesn't like its child list being changed while drawing
  ...
  private void showOpenCloseConfirmDialog(final boolean reopen) {
    @StringRes
    int messageResId =
        reopen ? R.string.reopen_issue_confirm : R.string.close_issue_confirm;
    @StringRes
    int buttonResId =
    ...
    new AlertDialog.Builder(this)
        ...
        .setPositiveButton(buttonResId,
                           (dialog, which) -> updateIssueState(reopen))
  ...
  private void updateIssueState(final boolean reopen) {
    ...
    IssueRequest request =
        IssueRequest.builder()
            ...
            .build();
    @StringRes
    int dialogResId = reopen ? R.string.opening_msg : R.string.closing_msg;
    @StringRes
    ...
    service.editIssue(mRepoOwner, mRepoName, mIssueNumber, request)
        ...
        .compose(RxUtils.wrapForBackgroundTask(
            this, dialogResId, getString(errorMessageResId, mIssueNumber)))
        .subscribe(result -> {
          mIssue = result;
