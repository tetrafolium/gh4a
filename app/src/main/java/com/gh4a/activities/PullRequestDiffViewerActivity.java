...
public class PullRequestDiffViewerActivity
    extends DiffViewerActivity<ReviewComment> {
  public static Intent
  makeIntent(final Context context, final String repoOwner,
             final String repoName, final int number, final String commitSha,
             final String path, final String diff,
             final List<ReviewComment> comments, final int initialLine,
    ...
    return DiffViewerActivity.fillInIntent(
        intent, repoOwner, repoName, commitSha, path, diff, comments,
        initialLine, highlightStartLine, highlightEndLine, highlightIsRight,
        initialComment);
  }
  ...
  protected void openCommentDialog(final long id, final long replyToId,
    ...
    Intent intent = EditPullRequestDiffCommentActivity.makeIntent(
  ...
  @Override
  protected Single<List<ReviewComment>>
  ...
  onUpdateReactions(final PositionalCommentBase comment,
    ...
    return ((ReviewComment)comment).toBuilder().reactions(reactions).build();
  }
  ...
  public Single<Reaction> addReaction(final ReactionBar.Item item,
    ...
    return service
        .createPullRequestReviewCommentReaction(mRepoOwner, mRepoName,
