...
public abstract class PagedDataBaseFragment<T> extends LoadingListFragmentBase
    implements RootAdapter.OnItemClickListener<T>,
               RootAdapter.OnScrolledToFooterListener {
  ...
  protected boolean shouldDoInitialLoad() { return true; }
        ...
        mPageSubject
            .flatMap(page
                     -> loadPage(page, force)
                            ...
                            .toObservable())
            .scan(Pair.create(new ArrayList<T>(), 0),
                  (pair, page) -> {
  ...
  private void fillData(final List<T> data, final Integer nextPage) {
    ...
    if (mAdapter.getCount() > 0 && !data.isEmpty() &&
        data.iterator().next() == mAdapter.getItem(0)) {
      // there are common items, preserve them in order to keep the scroll
      // position
  ...
  protected void
  onAddData(final RootAdapter<T, ? extends RecyclerView.ViewHolder> adapter,
