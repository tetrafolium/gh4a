...
public class CommitCompareFragment extends ListDataBaseFragment<Commit>
    implements RootAdapter.OnItemClickListener<Commit> {
  public static CommitCompareFragment newInstance(final String repoOwner,
  ...
  public static CommitCompareFragment
  newInstance(final String repoOwner, final String repoName,
              final int pullRequestNumber, final String baseRefLabel,
              final String baseRef, final String headRefLabel,
              final String headRef) {
  ...
  public void onItemClick(final Commit commit) {
    Intent intent = CommitActivity.makeIntent(
        getActivity(), mRepoOwner, mRepoName, mPullRequestNumber, commit.sha());
  ...
  protected Single<List<Commit>> onCreateDataSingle(final boolean bypassCache) {
    ...
    Single<CommitCompare> compareSingle =
        service.compareCommits(mRepoOwner, mRepoName, mBase, mHead)
            .map(ApiHelpers::throwOnFailure)
            .onErrorResumeNext(error -> {
              if (error instanceof ApiRequestException) {
                ApiRequestException are = (ApiRequestException)error;
                if (are.getStatus() == HttpURLConnection.HTTP_NOT_FOUND &&
                    mBaseLabel != null && mHeadLabel != null) {
                  // We got a 404; likely the history of the base branch was
                  // rewritten. Try the labels.
                  return service
                      .compareCommits(mRepoOwner, mRepoName, mBaseLabel,
                                      mHeadLabel)
                      .map(ApiHelpers::throwOnFailure);
                }
              }
