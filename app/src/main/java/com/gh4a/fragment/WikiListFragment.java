...
public class WikiListFragment extends ListDataBaseFragment<Feed>
    implements RootAdapter.OnItemClickListener<Feed> {
  private String mUserLogin;
  private String mRepoName;
  private String mInitialPage;

  public static WikiListFragment
  newInstance(final String owner, final String repo, final String initialPage) {
  ...
  protected Single<List<Feed>> onCreateDataSingle(final boolean bypassCache) {
    ...
    return SingleFactory
        .loadFeed(relativeUrl)
        // for empty repos, Github redirects to the repo's home page
        .compose(
            RxUtils.mapFailureToValue(HttpURLConnection.HTTP_MOVED_TEMP, empty))
        .compose(
            RxUtils.mapFailureToValue(HttpURLConnection.HTTP_NOT_FOUND, empty));
  }
  ...
  @Override
  protected void onAddData(final RootAdapter<Feed, ?> adapter,
  ...
  private void openViewer(final Feed feed) {
    startActivity(
        WikiActivity.makeIntent(getActivity(), mUserLogin, mRepoName, feed));
  }
}
