...
public class ReviewFragment extends ListDataBaseFragment<TimelineItem>
    implements TimelineItemAdapter.OnCommentAction, EditorBottomSheet.Callback,
               EditorBottomSheet.Listener {

  private static final int REQUEST_EDIT = 1000;
  private static final String EXTRA_SELECTED_REPLY_COMMENT_ID =
      "selected_reply_comment_id";

  @Nullable private TimelineItemAdapter mAdapter;
  private EditorBottomSheet mBottomSheet;

  public static ReviewFragment
  newInstance(final String repoOwner, final String repoName,
              final int issueNumber, final Review review,
              final IntentUtils.InitialCommentMarker mInitialComment) {
  ...
  public void onCreate(final @Nullable Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      mSelectedReplyCommentId =
          savedInstanceState.getLong(EXTRA_SELECTED_REPLY_COMMENT_ID);
    }
  }
  ...
  public boolean canChildScrollUp() {
    return (mBottomSheet != null && mBottomSheet.isExpanded()) ||
  ...
  onCreateDataSingle(final boolean bypassCache) {
    ...
    Single<TimelineItem.TimelineReview> reviewItemSingle =
        reviewService
            .getReview(mRepoOwner, mRepoName, mIssueNumber, mReview.id())
            .map(ApiHelpers::throwOnFailure)
            .map(TimelineItem.TimelineReview::new);

    Single<List<ReviewComment>> reviewCommentsSingle =
    ...
    Single<Boolean> hasCommentsSingle =
        reviewCommentsSingle.map(comments -> !comments.isEmpty());

    Single<Optional<List<GitHubFile>>> filesSingle =
        hasCommentsSingle.flatMap(hasComments -> {
          if (!hasComments) {
    ...
    Single<Optional<List<ReviewComment>>> commentsSingle =
        hasCommentsSingle.flatMap(hasComments -> {
          if (!hasComments) {
            return Single.just(Optional.absent());
          }
          return ApiHelpers.PageIterator
              .toSingle(page
                        -> commentService.getPullRequestComments(
    ...
    return Single.zip(
        reviewItemSingle, reviewCommentsSingle, filesSingle, commentsSingle,
        (reviewItem, reviewComments, filesOpt, commentsOpt) -> {
          if (!reviewComments.isEmpty()) {
            ...
            // Add all of the review comments to the review item creating
            // necessary diff hunks
            ...
            if (commentsOpt.isPresent()) {
              for (ReviewComment commitComment : commentsOpt.get()) {
                ...
                // Rest of the comments should be added only if they are under
                // the same diff hunks as the original review comments.
          ...
          List<TimelineItem.Diff> diffHunks =
              new ArrayList<>(reviewItem.getDiffHunks());
          ...
          for (TimelineItem.Diff diffHunk : diffHunks) {
            ...
            if (!diffHunk.isReply()) {
              items.add(
                  new TimelineItem.Reply(diffHunk.getInitialTimelineComment()));
            }
          }
  ...
  @Override
  protected RootAdapter<TimelineItem, ? extends RecyclerView.ViewHolder>
  onCreateAdapter() {
    mAdapter = new TimelineItemAdapter(getActivity(), mRepoOwner, mRepoName,
                                       mIssueNumber, true, false, this);
    return mAdapter;
  }

  @Override
  protected void onAddData(final RootAdapter<TimelineItem, ?> adapter,
  ...
  private void selectAndRemoveFirstReply(final List<TimelineItem> data) {
    ...
    for (TimelineItem timelineItem : data) {
      ...
      } else if (firstDiffItem != null && timelineItem instanceof
                                              TimelineItem.Reply) {
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      if (item instanceof TimelineItem.TimelineComment) {
        TimelineItem.TimelineComment comment =
            (TimelineItem.TimelineComment)item;
        if (mInitialComment.matches(comment.comment().id(),
  ...
  public void editComment(final GitHubCommentBase comment) {
    ...
    if (comment instanceof ReviewComment) {
      intent = EditPullRequestCommentActivity.makeIntent(
          getActivity(), mRepoOwner, mRepoName, mIssueNumber, comment.id(), 0L,
          comment.body(), 0);
    } else {
      intent = EditIssueCommentActivity.makeIntent(
          getActivity(), mRepoOwner, mRepoName, mIssueNumber, comment.id(),
          comment.body(), 0);
    }
  ...
  public void deleteComment(final GitHubCommentBase comment) {
    new AlertDialog.Builder(getActivity())
        ...
        .setPositiveButton(R.string.delete,
                           (dialog, which) -> handleDeleteComment(comment))
  ...
  @Override
  public Single<List<Reaction>>
  loadReactionDetails(final GitHubCommentBase comment,
    ...
    return ApiHelpers.PageIterator.toSingle(
        ...
        -> comment instanceof ReviewComment
               ...
               : service.getIssueCommentReactions(mRepoOwner, mRepoName,
                                                  comment.id(), page));
  }

  @Override
  public Single<Reaction> addReaction(final GitHubCommentBase comment,
    ...
    final Single<Response<Reaction>> responseSingle =
        comment instanceof ReviewComment
            ? service.createPullRequestReviewCommentReaction(
                  mRepoOwner, mRepoName, comment.id(), request)
            : service.createIssueCommentReaction(mRepoOwner, mRepoName,
                                                 comment.id(), request);
  ...
  public Single<?> onEditorDoSend(final String comment) {
    ...
    CreateReviewComment request = CreateReviewComment.builder()
                                      ...
                                      .build();
    return service
        .createReviewComment(mRepoOwner, mRepoName, mIssueNumber, request)
        .map(ApiHelpers::throwOnFailure);
  }
  ...
  private void handleDeleteComment(final GitHubCommentBase comment) {
    ...
    if (comment instanceof ReviewComment) {
      PullRequestReviewCommentService service =
          ServiceFactory.get(PullRequestReviewCommentService.class, false);
      responseSingle =
          service.deleteComment(mRepoOwner, mRepoName, comment.id());
    } else {
      IssueCommentService service =
          ServiceFactory.get(IssueCommentService.class, false);
      responseSingle =
          service.deleteIssueComment(mRepoOwner, mRepoName, comment.id());
    }
