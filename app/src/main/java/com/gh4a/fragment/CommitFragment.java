...
public class CommitFragment
    extends LoadingFragmentBase implements OnClickListener {
  public static CommitFragment newInstance(final String repoOwner,
    ...
    // Commit objects can be huge, depending on the number of patches attached
    // to it. In order to avoid TransactionTooLargeExceptions being thrown when
    // the activity we're attached to is recreated, store the data in compressed
    // form
  ...
  public interface CommentUpdateListener { void onCommentsUpdated(); }
  ...
  private void fillHeader() {
    ...
    if (author != null) {
      ...
      ivGravatar.setImageDrawable(
          new AvatarHandler.DefaultAvatarDrawable(null, email));
    }
    ...
    while (pos > 0 && pos < length &&
           Character.isWhitespace(message.charAt(pos))) {
      pos++;
    }
    ...
    tvTitle.setVisibility(StringUtils.isBlank(title) ? View.GONE
                                                     : View.VISIBLE);
    tvMessage.setVisibility(StringUtils.isBlank(message) ? View.GONE
                                                         : View.VISIBLE);
    ...
    tvTimestamp.setText(
        StringUtils.formatRelativeTime(activity, commit.author().date(), true));

    View committerContainer = mContentView.findViewById(R.id.committer);

    if (!ApiHelpers.authorEqualsCommitter(mCommit)) {
      ImageView commitGravatar =
          mContentView.findViewById(R.id.iv_commit_gravatar);
      ...
      String committerText = getString(
          R.string.commit_details, ApiHelpers.getCommitterName(app, mCommit),
          StringUtils.formatRelativeTime(activity, commit.committer().date(),
                                         true));
  ...
  protected void
  fillStats(final List<GitHubFile> files,
            final List<? extends PositionalCommentBase> comments) {
    ...
    int highlightColor =
        UiUtils.resolveColor(getActivity(), android.R.attr.textColorPrimary);
    ...
    for (int i = 0; i < count; i++) {
      ...
      ViewGroup fileView =
          (ViewGroup)inflater.inflate(R.layout.commit_filename, parent, false);
      ...
      if (file.patch() != null) {
        ...
        stats.append("\u00a0\u00a0\u00a0-")
            .append(String.valueOf(file.deletions()));
    ...
    tvSummary.setText(getString(R.string.commit_summary,
                                added + changed + renamed + deleted, additions,
                                deletions));
  }
  ...
  public void onClick(final View v) {
    if (v.getId() == R.id.iv_gravatar) {
      Intent intent =
          UserActivity.makeIntent(getActivity(), (String)v.getTag());
      if (intent != null) {
  ...
  protected void handleFileClick(final GitHubFile file) {
    ...
    if (FileUtils.isImage(file.filename())) {
      intent = FileViewerActivity.makeIntent(
          getActivity(), mRepoOwner, mRepoName, mObjectSha, file.filename());
    } else {
      intent = CommitDiffViewerActivity.makeIntent(
          getActivity(), mRepoOwner, mRepoName, mObjectSha, file.filename(),
          file.patch(), commentsForFile(file), -1, -1, false, null);
    }
