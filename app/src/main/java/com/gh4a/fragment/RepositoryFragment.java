...
public class RepositoryFragment extends LoadingFragmentBase
    implements OverviewRow.OnIconClickListener, View.OnClickListener {
  public static RepositoryFragment newInstance(final Repository repository,
                                               final String ref) {
  ...
  private static final String STATE_KEY_IS_README_EXPANDED =
      "is_readme_expanded";
  ...
  public void onActivityCreated(final Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      mIsReadmeExpanded =
          savedInstanceState.getBoolean(STATE_KEY_IS_README_EXPANDED, false);
      mIsReadmeLoaded =
          savedInstanceState.getBoolean(STATE_KEY_IS_README_LOADED, false);
    }
  ...
  private void fillData() {
    ...
    IntentSpan repoSpan = new IntentSpan(
    ...
    if (mRepository.isFork() && mRepository.parent() != null) {
      ...
      forkParentRow.setClickIntent(
          RepositoryActivity.makeIntent(getActivity(), parent));
    } else {
      forkParentRow.setVisibility(View.GONE);
    }

    OverviewRow privateRow = mContentView.findViewById(R.id.private_row);
    privateRow.setVisibility(mRepository.isPrivate() ? View.VISIBLE
                                                     : View.GONE);

    OverviewRow languageRow = mContentView.findViewById(R.id.language_row);
    languageRow.setVisibility(
        StringUtils.isBlank(mRepository.language()) ? View.GONE : View.VISIBLE);
    languageRow.setText(
        getString(R.string.repo_language, mRepository.language()));

    boolean showOverviewRowDivider =
        forkParentRow.getVisibility() == View.VISIBLE ||
    ...
    issuesRow.setClickIntent(
        IssueListActivity.makeIntent(getActivity(), owner, name));

    OverviewRow pullsRow = mContentView.findViewById(R.id.pulls_row);
    pullsRow.setClickIntent(
        IssueListActivity.makeIntent(getActivity(), owner, name, true));

    OverviewRow forksRow = mContentView.findViewById(R.id.forks_row);
    forksRow.setText(getResources().getQuantityString(
        R.plurals.fork, mRepository.forksCount(), mRepository.forksCount()));
    forksRow.setClickIntent(
        ForkListActivity.makeIntent(getActivity(), owner, name));

    mStarsRow = mContentView.findViewById(R.id.stars_row);
    mStarsRow.setIconClickListener(this);
    mStarsRow.setClickIntent(
        StargazerListActivity.makeIntent(getActivity(), owner, name));

    mWatcherRow = mContentView.findViewById(R.id.watchers_row);
    mWatcherRow.setIconClickListener(this);
    mWatcherRow.setClickIntent(
        WatcherListActivity.makeIntent(getActivity(), owner, name));
    ...
    mContentView.findViewById(R.id.tv_contributors_label)
  ...
  getForkedFromTextWithHighlight(final Repository parent) {
    ...
    spannableString.setSpan(
        colorSpan, forkedFromText.indexOf(parent.fullName()),
        forkedFromText.length(), Spanned.SPAN_INCLUSIVE_INCLUSIVE);
  ...
  private void fillTextView(final int id, final int stringId,
                            final String text) {
  ...
  private void updateStargazerUi() {
    mStarsRow.setText(getResources().getQuantityString(
        R.plurals.star, mRepository.stargazersCount(),
        mRepository.stargazersCount()));
    mStarsRow.setToggleState(mIsStarring != null && mIsStarring);
  }

  private void updateWatcherUi() {
    mWatcherRow.setText(getResources().getQuantityString(
        R.plurals.watcher, mRepository.subscribersCount(),
        mRepository.subscribersCount()));
  ...
  public void onClick(final View view) {
    ...
    if (id == R.id.tv_contributors_label) {
      ...
      intent = RepositoryActivity.makeIntent(getActivity(),
                                             (Repository)view.getTag());
    }
  ...
  private void updateReadmeVisibility() {
    mReadmeView.setVisibility(
        mIsReadmeExpanded && mIsReadmeLoaded ? View.VISIBLE : View.GONE);
    mLoadingView.setVisibility(
        mIsReadmeExpanded && !mIsReadmeLoaded ? View.VISIBLE : View.GONE);

    int drawableAttr =
        mIsReadmeExpanded ? R.attr.dropUpArrowIcon : R.attr.dropDownArrowIcon;
    int drawableRes = UiUtils.resolveDrawable(getContext(), drawableAttr);
    mReadmeTitleView.setCompoundDrawablesWithIntrinsicBounds(0, 0, drawableRes,
                                                             0);
  }
    ...
    service.getReadmeHtml(repoOwner, repoName, mRef)
        ...
        .map(htmlOpt -> {
          if (htmlOpt.isPresent()) {
            String html = HtmlUtils.rewriteRelativeUrls(
                htmlOpt.get(), repoOwner, repoName,
                mRef != null ? mRef : mRepository.defaultBranch());
  ...
  private void loadPullRequestCount(final boolean force) {
    SearchService service =
        ServiceFactory.get(SearchService.class, force, null, null, 1);
    String query =
        String.format(Locale.US, "type:pr repo:%s/%s state:open",
  ...
  private void toggleStarringState() {
    ...
    Single<Response<Void>> responseSingle =
        mIsStarring ? service.unstarRepository(mRepository.owner().login(),
                                               mRepository.name())
                    : service.starRepository(mRepository.owner().login(),
                                             mRepository.name());
        ...
        .subscribe(
            ...
            -> {
              if (mIsStarring != null) {
                ...
                mRepository =
                    mRepository.toBuilder()
                        .stargazersCount(mRepository.stargazersCount() +
                                         (mIsStarring ? 1 : -1))
                        .build();
                updateStargazerUi();
              }
  ...
  private void toggleWatchingState() {
    ...
    final String repoOwner = mRepository.owner().login(),
                 repoName = mRepository.name();
    ...
    if (mIsWatching) {
      ...
      SubscriptionRequest request =
          SubscriptionRequest.builder().subscribed(true).build();
      responseSingle =
          service.setRepositorySubscription(repoOwner, repoName, request)
              .map(ApiHelpers::throwOnFailure);
    }
        ...
        .subscribe(
            ...
            -> {
              if (mIsWatching != null) {
                ...
                mRepository =
                    mRepository.toBuilder()
                        .subscribersCount(mRepository.subscribersCount() +
                                          (mIsWatching ? 1 : -1))
                        .build();
                updateWatcherUi();
              }
  ...
  private void loadStarringState(final boolean force) {
    ...
    service
        .checkIfRepositoryIsStarred(mRepository.owner().login(),
                                    mRepository.name())
        // success response means 'starred'
        .map(ApiHelpers::mapToTrueOnSuccess)
        // 404 means 'not starred'
        .compose(
            RxUtils.mapFailureToValue(HttpURLConnection.HTTP_NOT_FOUND, false))
  ...
  private void loadWatchingState(final boolean force) {
    ...
    service
        .getRepositorySubscription(mRepository.owner().login(),
                                   mRepository.name())
        .map(ApiHelpers::throwOnFailure)
