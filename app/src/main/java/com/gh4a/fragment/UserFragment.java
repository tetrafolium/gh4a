...
public class UserFragment extends LoadingFragmentBase
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    mIsSelf =
        ApiHelpers.loginEquals(mUser, Gh4Application.get().getAuthLogin());
  ...
  private void fillData() {
    ...
    if (mUser.createdAt() != null) {
      joinDateRow.setText(
          getString(R.string.user_created_at,
                    DateFormat.getMediumDateFormat(getActivity())
                        .format(mUser.createdAt())));
    ...
    if (isUser) {
      ...
      followingRow.setText(getResources().getQuantityString(
          R.plurals.following, mUser.following(), mUser.following()));
    ...
    if (isUser) {
      int totalCount =
          orZero(mUser.publicGists()) + orZero(mUser.privateGists());
      gistsRow.setText(getResources().getQuantityString(
          R.plurals.gist, totalCount, totalCount));
      gistsRow.setClickIntent(
          GistListActivity.makeIntent(getActivity(), mUser.login()));
    }

    OverviewRow reposRow = mContentView.findViewById(R.id.repos_row);
    int repoCount =
        orZero(mUser.totalPrivateRepos()) + orZero(mUser.publicRepos());
    reposRow.setText(getResources().getQuantityString(R.plurals.repository,
                                                      repoCount, repoCount));
    reposRow.setClickIntent(RepositoryListActivity.makeIntent(
        getActivity(), mUser.login(), !isUser));

    TextView tvName = mContentView.findViewById(R.id.tv_name);
    String name =
        StringUtils.isBlank(mUser.name()) ? mUser.login() : mUser.name();
  ...
  private void updateFollowingUi() {
    ...
    @PluralsRes
    int resId = following ? R.plurals.follower_and_self : R.plurals.follower;
    int count = following ? mUser.followers() - 1 : mUser.followers();
    mFollowersRow.setText(
        getResources().getQuantityString(resId, count, count));
  ...
  public void onClick(final View view) {
    ...
    if (id == R.id.btn_repos) {
      intent = RepositoryListActivity.makeIntent(
          getActivity(), mUser.login(), mUser.type() == UserType.Organization);
  ...
  private void fillTopRepos(final Collection<Repository> topRepos) {
    ...
    if (topRepos != null && !topRepos.isEmpty()) {
      ...
      TextView hintView =
          (TextView)inflater.inflate(R.layout.hint_view, ll, false);
  ...
  private void fillOrganizations(final List<User> organizations) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      View rowView =
          inflater.inflate(R.layout.selectable_label_with_avatar, llOrg, false);
  ...
  private void toggleFollowingState() {
    UserFollowerService service =
        ServiceFactory.get(UserFollowerService.class, false);
    Single<Response<Void>> responseSingle =
        mIsFollowing ? service.unfollowUser(mUser.login())
  ...
  private void loadTopRepositories(final boolean force) {
    RepositoryService service =
        ServiceFactory.get(RepositoryService.class, force, null, null, 5);
    ...
    if (mIsSelf) {
      ...
      observable =
          service.getOrganizationRepositories(mUser.login(), filterData, 1);
    } else {
    ...
    observable.map(ApiHelpers::throwOnFailure)
        ...
        .doOnSubscribe(disposable -> {
          mContentView.findViewById(R.id.pb_top_repos)
  ...
  private void loadOrganizationsIfUser(final boolean force) {
    ...
    ApiHelpers.PageIterator
        .toSingle(page
                  -> mIsSelf ? service.getMyOrganizations(page)
                             : service.getUserPublicOrganizations(mUser.login(),
                                                                  page))
  ...
  private void loadOrganizationMemberCountIfOrg(final boolean force) {
    ...
    ApiHelpers.PageIterator
        ...
        .subscribe(count -> {
          OverviewRow membersRow = mContentView.findViewById(R.id.members_row);
          membersRow.setText(
              getResources().getQuantityString(R.plurals.member, count, count));
  ...
  private void loadIsFollowingStateIfNeeded(final boolean force) {
    ...
    UserFollowerService service =
        ServiceFactory.get(UserFollowerService.class, force);
