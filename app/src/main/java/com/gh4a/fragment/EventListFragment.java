...
public abstract class EventListFragment
    extends PagedDataBaseFragment<GitHubEvent> {
  ...
  private static final GitHubEventType[] REPO_EVENTS = new GitHubEventType[] {
      GitHubEventType.PushEvent,        GitHubEventType.IssuesEvent,
      GitHubEventType.WatchEvent,       GitHubEventType.CreateEvent,
      GitHubEventType.PullRequestEvent, GitHubEventType.CommitCommentEvent,
      GitHubEventType.DeleteEvent,      GitHubEventType.DownloadEvent,
      GitHubEventType.ForkApplyEvent,   GitHubEventType.PublicEvent,
      GitHubEventType.MemberEvent,      GitHubEventType.IssueCommentEvent};
  ...
  @Override
  protected RootAdapter<GitHubEvent, ? extends RecyclerView.ViewHolder>
  onCreateAdapter() {
  ...
  public void onItemClick(final GitHubEvent event) {
    ...
    if (Arrays.binarySearch(REPO_EVENTS, event.type()) >= 0 &&
        eventRepo == null) {
      Toast
          .makeText(getActivity(), R.string.repo_not_found_toast,
                    Toast.LENGTH_LONG)
          .show();
      return;
    }
    ...
    case CommitCommentEvent: {
      ...
      if (comment != null) {
        intentSingle = CommitCommentLoadTask.load(
            getActivity(), repoOwner, repoName, comment.commitId(),
    ...
    case DeleteEvent:
      intent =
          RepositoryActivity.makeIntent(getActivity(), repoOwner, repoName);
      break;

    case DownloadEvent: {
      DownloadPayload payload = (DownloadPayload)event.payload();
      Download download = payload.download();
      UiUtils.enqueueDownloadWithPermissionCheck(
          (BaseActivity)getActivity(), download.htmlUrl(),
          download.contentType(), download.name(), download.description(),
          null);
      break;
    }
    ...
    case GollumEvent: {
      ...
      intent = WikiListActivity.makeIntent(
          getActivity(), repoOwner, repoName,
    ...
    case IssueCommentEvent: {
      ...
      IntentUtils.InitialCommentMarker initialComment =
          payload.comment() != null
              ? new IntentUtils.InitialCommentMarker(payload.comment().id())
              : null;

      if (request != null && request.htmlUrl() != null) {
        intent = PullRequestActivity.makeIntent(
            getActivity(), repoOwner, repoName, issue.number(),
            initialComment != null ? PullRequestActivity.PAGE_CONVERSATION : -1,
            initialComment);
      } else if (issue != null) {
        intent = IssueActivity.makeIntent(getActivity(), repoOwner, repoName,
                                          issue.number(), initialComment);
      }
      break;
    }
    ...
    case PullRequestEvent: {
      ...
      intent = PullRequestActivity.makeIntent(getActivity(), repoOwner,
                                              repoName, payload.number());
      break;
    }
    ...
    case PullRequestReviewCommentEvent: {
      ...
      IntentUtils.InitialCommentMarker initialComment =
          comment != null ? new IntentUtils.InitialCommentMarker(comment.id())
                          : null;

      if (pr != null) {
        if (initialComment != null) {
          intentSingle = PullRequestReviewCommentLoadTask.load(
              getActivity(), repoOwner, repoName, pr.number(), initialComment);
        } else {
          intent = PullRequestActivity.makeIntent(
              getActivity(), repoOwner, repoName, pr.number(), -1, null);
        }
    ...
    case PushEvent: {
      ...
      if (commits != null && !commits.isEmpty()) {
        if (commits.size() > 1) {
          ...
          intent =
              CompareActivity.makeIntent(getActivity(), repoOwner, repoName,
    ...
    case ReleaseEvent: {
      ...
      if (release != null) {
        intent = ReleaseInfoActivity.makeIntent(getActivity(), repoOwner,
                                                repoName, release.id());
      }
      break;
    }
    ...
    if (intent != null) {
      ...
      intentSingle.compose(RxUtils::doInBackground)
          .compose(RxUtils.wrapWithProgressDialog(getActivity(),
          ...
          .subscribe(
              ...
              error
              -> Log.d(Gh4Application.LOG_TAG, "Loading click intent failed",
                       error));
    }
  }
  ...
  public void onCreateContextMenu(final ContextMenu menu, final View v,
    ...
    if (repoOwner != null) {
      menu.add(getString(R.string.menu_repo, repoOwner + "/" + repoName))
          .setIntent(RepositoryActivity.makeIntent(getActivity(), repoOwner,
                                                   repoName));
    }
    ...
    case CommitCommentEvent:
      if (repoOwner != null) {
        ...
        menu.add(getString(R.string.menu_commit, sha.substring(0, 7)))
            .setIntent(CommitActivity.makeIntent(getActivity(), repoOwner,
                                                 repoName, sha));
      }
      break;
    ...
    case ForkEvent: {
      ...
      Repository forkee = payload.forkee();
      if (forkee != null) {
        menu.add(getString(R.string.menu_fork,
                           forkee.owner().login() + "/" + forkee.name()))
    ...
    case GollumEvent: {
      ...
      if (pages != null &&
          !pages.isEmpty()) { // TODO: now just open the first page
        menu.add(getString(R.string.menu_wiki))
            .setIntent(new Intent(Intent.ACTION_VIEW,
                                  Uri.parse(pages.get(0).htmlUrl())));
      }
      break;
    }
    ...
    case IssueCommentEvent: {
      ...
      menu.add(getString(isPullRequest ? R.string.menu_pulls
                                       : R.string.menu_issues))
          .setIntent(IssueListActivity.makeIntent(getActivity(), repoOwner,
                                                  repoName, isPullRequest));
      break;
    }
    ...
    case IssuesEvent: {
      ...
      menu.add(getString(R.string.menu_issue, issueNumber))
          .setIntent(IssueActivity.makeIntent(getActivity(), repoOwner,
    ...
    case PullRequestEvent: {
      ...
      menu.add(getString(R.string.menu_pull, payload.number()))
          .setIntent(PullRequestActivity.makeIntent(
              getActivity(), repoOwner, repoName, payload.number()));
      break;
    }
    ...
    case PushEvent: {
      if (repoOwner != null) {
        PushPayload payload = (PushPayload)event.payload();
        menu.add(getString(R.string.menu_compare,
                           payload.head().substring(0, 7)))
            .setIntent(CompareActivity.makeIntent(getActivity(), repoOwner,
                                                  repoName, payload.before(),
                                                  payload.head()));
  ...
  public boolean onContextItemSelected(final MenuItem item) {
    ...
    if (id >= MENU_DOWNLOAD_START && id <= MENU_DOWNLOAD_END) {
      if (event.type() == GitHubEventType.ReleaseEvent) {
        ReleasePayload payload = (ReleasePayload)event.payload();
        ReleaseAsset asset =
            payload.release().assets().get(id - MENU_DOWNLOAD_START);
        UiUtils.enqueueDownloadWithPermissionCheck(
            (BaseActivity)getActivity(), asset.url(), asset.contentType(),
