...
public class RepositoryEventListFragment extends EventListFragment {
  ...
  public static RepositoryEventListFragment
  newInstance(final Repository repository) {
  ...
  loadPage(final int page, final boolean bypassCache) {
    ...
    return service.getRepositoryEvents(mRepository.owner().login(),
                                       mRepository.name(), page);
  }
}
