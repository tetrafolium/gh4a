...
public class GistListFragment extends PagedDataBaseFragment<Gist>
    implements RootAdapter.OnItemClickListener<Gist> {
  public static GistListFragment newInstance(final String userLogin,
  ...
  @Override
  protected Single<Response<Page<Gist>>> loadPage(final int page,
                                                  final boolean bypassCache) {
    final GistService service =
        ServiceFactory.get(GistService.class, bypassCache);
    return mShowStarred ? service.getUserStarredGists(page)
                        : service.getUserGists(mUserLogin, page);
  }

  @Override
  protected RootAdapter<Gist, ? extends RecyclerView.ViewHolder>
  onCreateAdapter() {
  ...
  protected int getEmptyTextResId() {
    return mShowStarred ? R.string.no_starred_gists_found
