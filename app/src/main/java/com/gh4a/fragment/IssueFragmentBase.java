...
public abstract class IssueFragmentBase
    extends ListDataBaseFragment<TimelineItem>
    implements View.OnClickListener, TimelineItemAdapter.OnCommentAction,
               EditorBottomSheet.Callback, EditorBottomSheet.Listener,
               ReactionBar.Callback, ReactionBar.Item,
               ReactionBar.ReactionDetailsCache.Listener {
  protected static final int REQUEST_EDIT = 1000;

  protected static final List<IssueEventType> INTERESTING_EVENTS =
      Arrays.asList(
          IssueEventType.Closed, IssueEventType.Reopened, IssueEventType.Merged,
          IssueEventType.Referenced, IssueEventType.Assigned,
          IssueEventType.Unassigned, IssueEventType.Labeled,
          IssueEventType.Unlabeled, IssueEventType.Locked,
          IssueEventType.Unlocked, IssueEventType.Milestoned,
          IssueEventType.Demilestoned, IssueEventType.Renamed,
          IssueEventType.HeadRefDeleted, IssueEventType.HeadRefRestored,
  ...
  buildArgs(final String repoOwner, final String repoName, final Issue issue,
  ...
  protected void onRecyclerViewInflated(final RecyclerView view,
    ...
    mListHeaderView =
        inflater.inflate(R.layout.issue_comment_list_header, view, false);
  ...
  public void onCreateOptionsMenu(final Menu menu,
    ...
    if (mReactionMenuHelper == null) {
      mReactionMenuHelper = new ReactionBar.AddReactionMenuHelper(
          getActivity(), reactItem.getSubMenu(), this, this,
  ...
  onCreateAdapter() {
    mAdapter = new TimelineItemAdapter(
      ...
      for (int i = 0; i < data.size(); i++) {
        ...
        if (item instanceof TimelineItem.TimelineComment) {
          ...
          TimelineItem.TimelineReview review =
              (TimelineItem.TimelineReview)item;
          if (mInitialComment.matches(review.review().id(),
                                      review.getCreatedAt())) {
            scrollToAndHighlightPosition(i + 1 /* adjust for header view */);
            break;
          }
  ...
  private void fillData() {
    ...
    tvTimestamp.setText(StringUtils.formatRelativeTime(
        getActivity(), mIssue.createdAt(), true));
    ...
    if (!StringUtils.isBlank(body)) {
      ...
      SpannableString noDescriptionString =
          new SpannableString(getString(R.string.issue_no_description));
      noDescriptionString.setSpan(new StyleSpan(Typeface.ITALIC), 0,
                                  noDescriptionString.length(), 0);
      descriptionView.setText(noDescriptionString);
    }

    View milestoneGroup =
        mListHeaderView.findViewById(R.id.milestone_container);
    ...
    if (assignees != null && !assignees.isEmpty()) {
      ViewGroup assigneeContainer =
          mListHeaderView.findViewById(R.id.assignee_list);
      LayoutInflater inflater = getLayoutInflater();
      assigneeContainer.removeAllViews();
      for (User assignee : assignees) {
        View row =
            inflater.inflate(R.layout.row_assignee, assigneeContainer, false);
  ...
  public Single<List<Reaction>> loadReactionDetails(final ReactionBar.Item item,
    ...
    return ApiHelpers.PageIterator.toSingle(
        page
        -> service.getIssueReactions(mRepoOwner, mRepoName, mIssue.number(),
                                     page));
  }
  ...
  public Single<Reaction> addReaction(final ReactionBar.Item item,
    ...
    return service
        .createIssueReaction(mRepoOwner, mRepoName, mIssue.number(), request)
  ...
  loadReactionDetails(final GitHubCommentBase comment,
    ...
    return ApiHelpers.PageIterator.toSingle(
        page
        -> service.getIssueCommentReactions(mRepoOwner, mRepoName, comment.id(),
                                            page));
  }
  ...
  public Single<Reaction> addReaction(final GitHubCommentBase comment,
    ...
    ReactionRequest request =
        ReactionRequest.builder().content(content).build();
    return service
        .createIssueCommentReaction(mRepoOwner, mRepoName, comment.id(),
                                    request)
        .map(ApiHelpers::throwOnFailure);
  }
  ...
  public Single<?> onEditorDoSend(final String comment) {
    ...
    return service
        .createIssueComment(mRepoOwner, mRepoName, mIssue.number(), request)
        .map(ApiHelpers::throwOnFailure);
  }
  ...
  public String getShareSubject(final GitHubCommentBase comment) {
    return getString(R.string.share_comment_subject, comment.id(),
                     mIssue.number(), mRepoOwner + "/" + mRepoName);
  }
  ...
  protected abstract Single<Response<Void>>
  doDeleteComment(GitHubCommentBase comment);
