...
public class NotificationListFragment extends LoadingListFragmentBase
    implements RootAdapter.OnItemClickListener<NotificationHolder>,
               NotificationAdapter.OnNotificationActionCallback {
  public static final String EXTRA_INITIAL_REPO_OWNER =
      "initial_notification_repo_owner";
  public static final String EXTRA_INITIAL_REPO_NAME =
      "initial_notification_repo_name";
  ...
  public void onItemClick(final NotificationHolder item) {
    ...
    if (item.notification == null) {
      ...
      if (url != null) {
        ...
        intent = BrowseFilter.makeRedirectionIntent(
            getActivity(), uri,
            new IntentUtils.InitialCommentMarker(
                item.notification.lastReadAt()));
      } else {
        intent = null;
      }
    }
    ...
    case R.id.mark_all_as_read:
      new AlertDialog.Builder(getActivity())
          ...
          .setPositiveButton(R.string.mark_all_as_read,
                             (dialog, which) -> markAsRead(null, null))
    ...
    if (notificationHolder.notification == null) {
      ...
      new AlertDialog.Builder(getActivity())
          ...
          .setPositiveButton(R.string.mark_as_read,
                             (dialog, which) -> markAsRead(repository, null))
          .setNegativeButton(R.string.cancel, null)
          .show();
    } else {
  ...
  public void unsubscribe(final NotificationHolder notificationHolder) {
    ...
    NotificationService service =
        ServiceFactory.get(NotificationService.class, false);
    SubscriptionRequest request =
        SubscriptionRequest.builder().ignored(true).build();
  ...
  private void updateMenuItemVisibility() {
    ...
    mMarkAllAsReadMenuItem.setVisible(isContentShown() &&
                                      mAdapter.hasUnreadNotifications());
  }

  private void
  scrollToInitialNotification(final List<NotificationHolder> notifications) {
  ...
  private void markAsRead(final Repository repository,
    ...
    responseSingle.map(ApiHelpers::mapToBooleanOrThrowOnFailure)
        ...
        .subscribe(
            result
            -> handleMarkAsRead(repository, notification),
            error
            -> handleActionFailure("Mark notifications as read failed", error));
  }
