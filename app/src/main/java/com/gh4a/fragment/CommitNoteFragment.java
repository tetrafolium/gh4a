...
public class CommitNoteFragment extends ListDataBaseFragment<GitComment>
    implements CommitNoteAdapter.OnCommentAction<GitComment>,
               EditorBottomSheet.Callback, EditorBottomSheet.Listener {

  public static CommitNoteFragment
  newInstance(final String repoOwner, final String repoName,
              final String commitSha, final Commit commit,
              final List<GitComment> allComments,
  ...
  public void onViewCreated(final View view, final Bundle savedInstanceState) {
    ...
    mBottomSheet.post(() -> {
      // Fix an issue where the bottom sheet is initially located outside of the
      // visible screen area
  ...
  @Override
  protected RootAdapter<GitComment, ? extends RecyclerView.ViewHolder>
  onCreateAdapter() {
    mAdapter =
        new CommitNoteAdapter(getActivity(), mRepoOwner, mRepoName, this);
    return mAdapter;
  }

  @Override
  protected void onAddData(final RootAdapter<GitComment, ?> adapter,
                           final List<GitComment> data) {
    ...
    if (mInitialComment != null) {
      for (int i = 0; i < data.size(); i++) {
        if (mInitialComment.matches(data.get(i).id(),
  ...
  onCreateDataSingle(final boolean bypassCache) {
    ...
    return ApiHelpers.PageIterator
        .toSingle(page
                  -> service.getCommitComments(mRepoOwner, mRepoName,
                                               mObjectSha, page))
  ...
  protected List<GitComment> onGetInitialData() {
    List<GitComment> comments =
        getArguments().getParcelableArrayList("comments");
  ...
  public void editComment(final GitComment comment) {
    Intent intent = EditCommitCommentActivity.makeIntent(
        getActivity(), mRepoOwner, mRepoName, mObjectSha, comment.id(),
        comment.body());
  ...
  public void deleteComment(final GitComment comment) {
    new AlertDialog.Builder(getActivity())
        ...
        .setPositiveButton(R.string.delete,
                           (dialog, which) -> deleteComment(comment.id()))
  ...
  public Single<?> onEditorDoSend(final String comment) {
    ...
    CreateCommitComment request =
        CreateCommitComment.builder().body(comment).build();
    return service
        .createCommitComment(mRepoOwner, mRepoName, mObjectSha, request)
