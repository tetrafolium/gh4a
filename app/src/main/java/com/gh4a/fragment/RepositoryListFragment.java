...
public class RepositoryListFragment extends PagedDataBaseFragment<Repository> {
  ...
  public static RepositoryListFragment
  newInstance(final String login, final boolean isOrg, final String repoType,
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    final boolean isSelf =
        ApiHelpers.loginEquals(mLogin, Gh4Application.get().getAuthLogin());

    // We're operating on the limit of what Github's repo API supports.
    // Specifically, it doesn't support sorting for the organization repo list
    // endpoint, so we're using the user repo list endpoint for organizations as
    // well. Doing so has a few quirks though:
    // - the 'all' filter returns an empty list when querying organization
    // repos, so we
    //   need to omit the filter in that case
    // - 'sources' and 'forks' filter types are only supported for the org repo
    // list endpoint,
    //   but not for the user repo list endpoint, hence we emulate it by
    //   querying for 'all' and filtering the result
    // Additionally, using affiliation together with type is not supported, so
    // omit type when adding affiliation.

    String actualFilterType =
        "sources".equals(mRepoType) || "forks".equals(mRepoType) ? "all"
                                                                 : mRepoType;

    if (isSelf && TextUtils.equals(actualFilterType, "all")) {
      mFilterData.put("affiliation", "owner,collaborator");
    } else if (!TextUtils.equals(actualFilterType, "all") ||
               !args.getBoolean("is_org")) {
  ...
  protected void onAddData(
      final RootAdapter<Repository, ? extends RecyclerView.ViewHolder> adapter,
  ...
  loadPage(final int page, final boolean bypassCache) {
    final RepositoryService service =
        ServiceFactory.get(RepositoryService.class, bypassCache);
