...
public class HtmlUtils {
  private static class ReplySpan implements LeadingMarginSpan {
    ...
    public void drawLeadingMargin(final Canvas c, final Paint p, final int x,
                                  final int dir, final int top,
                                  final int baseline, final int bottom,
                                  final CharSequence text, final int start,
                                  final int end, final boolean first,
                                  final Layout layout) {
  ...
  private static class CodeBlockSpan implements LineBackgroundSpan {
    ...
    public CodeBlockSpan(final int color) { mColor = color; }
   ...
   * @return
   */
  public static String rewriteRelativeUrls(final String html,
                                           final String repoUser,
                                           final String repoName,
                                           final String branch) {
    final String baseUrl =
        "https://raw.github.com/" + repoUser + "/" + repoName + "/" + branch;
    ...
    while (m.find()) {
      ...
      m.appendReplacement(
          sb, Matcher.quoteReplacement(m.group(1) + "=\"" + url + "\""));
    }
    m.appendTail(sb);
  ...
  /* a copy of the framework's HTML class, stripped down and extended for our
   * use cases */
  private static class Html {
    ...
    fromHtml(final Context context, final String source,
             final android.text.Html.ImageGetter imageGetter) {
  ...
  private static class HtmlToSpannedConverter implements ContentHandler {
    ...
    public HtmlToSpannedConverter(
        final Context context, final String source,
    ...
    public Spanned convert() {
      ...
      // Replace the placeholders for leading margin spans in reverse order, so
      // the leading margins are drawn in order of tag start
      Object[] obj = mSpannableStringBuilder.getSpans(
          0, mSpannableStringBuilder.length(), NeedsReversingSpan.class);
      ...
      obj = mSpannableStringBuilder.getSpans(
      ...
      for (Object span : obj) {
        ...
        if (end == start) {
          ...
          mSpannableStringBuilder.setSpan(span, start, end,
    ...
    private void handleStartTag(final String tag, final Attributes attributes) {
      ...
      if (tag.equalsIgnoreCase("br")) {
        ...
        // </br> for each <br> so we can safely emit the linebreaks when we
        ...
        start(mSpannableStringBuilder,
              new List(parseIntAttribute(attributes, "start", 1)));
      } else if (tag.equalsIgnoreCase("li")) {
        startLi(mSpannableStringBuilder, attributes);
      } else if (tag.equalsIgnoreCase("input")) {
        if ("checkbox".equalsIgnoreCase(attributes.getValue("", "type"))) {
          @AttrRes
          int drawableAttrResId = attributes.getIndex("", "checked") >= 0
                                      ? R.attr.checkboxCheckedSmallIcon
                                      : R.attr.checkboxUncheckedSmallIcon;
          Drawable d = ContextCompat.getDrawable(
              mContext, UiUtils.resolveDrawable(mContext, drawableAttrResId));
          d.setBounds(0, 0, d.getIntrinsicWidth(), d.getIntrinsicHeight());
          ImageSpan span = new ImageSpan(d, ImageSpan.ALIGN_BOTTOM);

          mSpannableStringBuilder.append("  ");
          mSpannableStringBuilder.setSpan(span,
                                          mSpannableStringBuilder.length() - 2,
                                          mSpannableStringBuilder.length() - 1,
        ...
        HorizontalLineSpan span =
            new HorizontalLineSpan(mDividerHeight, 0x60aaaaaa);
        ...
        mSpannableStringBuilder.setSpan(span, len - 1, len,
                                        Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
    ...
    private void handleEndTag(final String tag) {
      if (tag.equalsIgnoreCase("br")) {
        ...
        end(mSpannableStringBuilder, Strikethrough.class,
    ...
    private static void appendNewlines(final Editable text,
                                       final int minNewline) {
    ...
    private static void startBlockElement(final Editable text,
    ...
    private static void handleBr(final Editable text) { text.append('\n'); }
    ...
    private void startBlockquote(final Editable text,
                                 final Attributes attributes) {
    ...
    private void endBlockquote(final Editable text) {
      endBlockElement(text);
      end(text, Blockquote.class,
          new ReplySpan(mReplyMargin, mReplyMarkerSize, 0xffdddddd));
    }

    private void startHeading(final Editable text, final Attributes attributes,
                              final int level) {
      startBlockElement(text, attributes);
    ...
    private static void setSpanFromMark(final Spannable text, final Object mark,
                                        final Object... spans) {
    ...
    private static void end(final Editable text, final Class<?> kind,
                            final Object repl) {
    ...
    private void startCssStyle(final Editable text,
                               final Attributes attributes) {
      String style = attributes.getValue("", "style");
      if (style != null) {
    ...
    private static void startImg(final Editable text,
                                 final Attributes attributes,
    ...
    private static void startA(final Editable text,
                               final Attributes attributes) {
    ...
    private static int parseIntAttribute(final Attributes attributes,
                                         final String name,
    ...
    private static class Code {
      ...
      public Code(final boolean inPre) { mInPre = inPre; }
    }
    ...
    private static class Font {
      ...
      public Font(final String face) { mFace = face; }
    }

    private static class Href {
      public final String mHref;

      public Href(final String href) { mHref = href; }
    }
    ...
    private static class Heading {
      ...
      public Heading(final int level) { mLevel = level; }
    }

    private static class Newline {
      private final int mNumNewlines;

      public Newline(final int numNewlines) { mNumNewlines = numNewlines; }
    }
