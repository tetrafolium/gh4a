...
public class IntentUtils {
  ...
  public static void launchBrowser(final Context context, final Uri uri,
    ...
    if (intent != null) {
      ...
      Toast.makeText(context, R.string.no_browser_found, Toast.LENGTH_LONG)
          .show();
    }
  }
  ...
  private static Intent createBrowserIntent(final Context context,
                                            final Uri uri) {
  ...
  public static Intent createViewerOrBrowserIntent(final Context context,
                                                   final Uri uri,
                                                   final String mime) {
    final Uri dummyUri = buildDummyUri(uri);
    final Intent viewIntent =
        new Intent(Intent.ACTION_VIEW).setDataAndType(dummyUri, mime);
    final Intent resolvedViewIntent =
        createActivityChooserIntent(context, viewIntent, uri);
  ...
  public static void openInCustomTabOrBrowser(final Activity activity,
                                              final Uri uri) {
    String pkg = CustomTabsHelper.getPackageNameToUse(activity);
    if (pkg != null) {
      int color = UiUtils.resolveColor(activity, R.attr.colorPrimary);
      CustomTabsIntent i =
          new CustomTabsIntent.Builder().setToolbarColor(color).build();
  ...
  public static Uri.Builder createBaseUriForRepo(final String repoOwner,
                                                 final String repoName) {
    return createBaseUriForUser(repoOwner).appendPath(repoName);
  }

  public static void share(final Context context, final String subject,
                           final Uri url) {
    ...
    context.startActivity(Intent.createChooser(
        shareIntent, context.getString(R.string.share_title)));
  }
  ...
  private static Intent createActivityChooserIntent(final Context context,
                                                    final Intent intent,
                                                    final Uri uri) {
  ...
  public static void startNewTask(final @NonNull Context context,
  ...
  public static void
  putParcelableToBundleCompressed(final Bundle bundle, final String key,
  ...
  public static <T> T readCompressedParcelableFromBundle(final Bundle bundle,
                                                         final String key) {
  ...
  public static class InitialCommentMarker implements Parcelable {
    ...
    public InitialCommentMarker(final long commentId) { this(commentId, null); }

    public InitialCommentMarker(final Date date) { this(-1, date); }
    ...
    public static final Parcelable.Creator<InitialCommentMarker> CREATOR =
        new Parcelable.ClassLoaderCreator<InitialCommentMarker>() {
          @Override
          public InitialCommentMarker createFromParcel(
          ...
          public InitialCommentMarker createFromParcel(final Parcel in) {
            ...
            return new InitialCommentMarker(
                commentId, timeMillis != -1 ? new Date(timeMillis) : null);
          }
          @Override
