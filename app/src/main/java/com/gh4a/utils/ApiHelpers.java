...
public class ApiHelpers {
  ...
  public static final Comparator<GitHubCommentBase> COMMENT_COMPARATOR =
      (lhs, rhs) -> {
  ...
  // RepositoryCommit
  public static String getAuthorName(final Context context,
                                     final Commit commit) {
  ...
  public static String getCommitterName(final Context context,
                                        final Commit commit) {
  ...
  public static boolean authorEqualsCommitter(final Commit commit) {
    if (commit.committer() != null && commit.author() != null) {
      return TextUtils.equals(commit.committer().login(),
                              commit.author().login());
    }
  ...
  public static Uri normalizeUri(final Uri uri) {
    ...
    return uri.buildUpon().path(path).authority(authority).build();
  }
  ...
  public static <T> T throwOnFailure(final Response<T> response)
  ...
  public static boolean mapToTrueOnSuccess(final Response<Void> response)
      throws ApiRequestException {
    if (response.code() == HttpURLConnection.HTTP_UNAUTHORIZED) {
      Gh4Application.get().logout();
    }
  ...
  mapToBooleanOrThrowOnFailure(final Response<Void> response)
      throws ApiRequestException {
    if (response.isSuccessful()) {
      return true;
  ...
  public static class SearchPageAdapter<U, D> extends Page<D> {
    ...
    public SearchPageAdapter(final SearchPage<U> page,
  ...
  public static class PageIterator<T> {
    ...
    public static <T> Observable<List<T>>
    toObservable(final PageProducer<T> producer) {
      ...
      return pageControl.concatMap(page -> {
        if (page.isPresent()) {
          return producer.getPage(page.get())
              ...
              .doOnNext(
                  resultPage
                  -> pageControl.onNext(Optional.ofWithNull(resultPage.next())))
              .map(responsePage -> responsePage.items());
        } else {
          return Observable.<List<T>>empty().doOnComplete(
    ...
    public static <T> Single<List<T>> toSingle(final PageProducer<T> producer) {
      return toObservable(producer).toList().map(lists -> {
    ...
    private static <T> Observable<Page<T>>
    evaluateError(final Observable<Response<Page<T>>> upstream) {
