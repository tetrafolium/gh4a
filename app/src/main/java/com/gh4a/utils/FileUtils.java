...
public class FileUtils {
  private static final List<String> MARKDOWN_EXTS =
      Arrays.asList("markdown", "md", "mdown", "mkdn", "mkd");

  private static final HashMap<String, String> MIME_TYPE_OVERRIDES =
      new HashMap<>();
  static {
    // .ts can be both a TypeScript file and a MPEG2 transport stream file. As
    // the former is the more likely case for us and the framework returns the
    // latter, override to assume a text file.
    MIME_TYPE_OVERRIDES.put("ts", "text/x-typescript");
    // there is no general MIME type mapping for Java properties files, but
    // they're text
  ...
  private static boolean isExtensionIn(final String filename,
