...
public class UiUtils {
  public static final LinkMovementMethod CHECKING_LINK_METHOD =
      new LinkMovementMethod() {
        @Override
        public boolean onTouchEvent(final @NonNull TextView widget,
                                    final @NonNull Spannable buffer,
                                    final @NonNull MotionEvent event) {
          try {
            return super.onTouchEvent(widget, buffer, event);
          } catch (ActivityNotFoundException e) {
            Toast
                .makeText(widget.getContext(), R.string.link_not_openable,
                          Toast.LENGTH_LONG)
                .show();
            return true;
  ...
  public static int textColorForBackground(final Context context,
  ...
  public static void trySetListOverscrollColor(final RecyclerView view,
                                               final int color) {
  ...
  @TargetApi(21)
  private static class RecyclerViewEdgeColorHelper
    ...
    private boolean ensureStaticMethodsAndFields() {
      ...
      try {
        sTopEnsureMethod =
            RecyclerView.class.getDeclaredMethod("ensureTopGlow");
        sTopEnsureMethod.setAccessible(true);
        sBottomEnsureMethod =
            RecyclerView.class.getDeclaredMethod("ensureBottomGlow");
  ...
  public static int mixColors(final int startColor, final int endColor,
  ...
  public static int resolveDrawable(final Context context,
                                    final @AttrRes int styledAttributeId) {
    TypedArray a =
        context.obtainStyledAttributes(new int[] {styledAttributeId});
    int resource = a.getResourceId(0, 0);
    a.recycle();
    return resource;
  }

  public static @ColorInt int resolveColor(final Context context, final
                                           @AttrRes int styledAttributeId) {
    TypedArray a =
        context.obtainStyledAttributes(new int[] {styledAttributeId});
  ...
  private static void
  enqueueDownload(final Context context, final Uri uri, final String fileName,
                  final String description, final String mimeType,
                  final String mediaType, final boolean wifiOnly) {
  ...
  public static void enqueueDownloadWithPermissionCheck(
      final BaseActivity activity, final String url, final String mimeType,
    ...
    final ActivityCompat.OnRequestPermissionsResultCallback cb =
      ...
      if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
        enqueueDownload(activity, url, mimeType, fileName, description,
                        mediaType);
      }
    };
  ...
  private static void enqueueDownload(final Context context, final String url,
    ...
    final Uri uri = Uri.parse(url)
                        ...
                        .appendQueryParameter(
                            "access_token", Gh4Application.get().getAuthToken())
                        .build();

    if (!downloadNeedsWarning(context)) {
      enqueueDownload(context, uri, fileName, description, mimeType, mediaType,
                      false);
      return;
    }
  ...
  public static abstract class EmptinessWatchingTextWatcher
  ...
  public static class ButtonEnableTextWatcher
      extends EmptinessWatchingTextWatcher {
  ...
  public static abstract class QuoteActionModeCallback
      implements ActionMode.Callback {
    private final TextView mView;

    public QuoteActionModeCallback(final TextView view) { mView = view; }
  ...
  formatLabelList(final Context context, final List<Label> labels) {
  ...
  public static int limitViewHeight(final int heightMeasureSpec,
                                    final int maxHeight) {
  ...
  public static void setMenuItemText(final Context context, final MenuItem item,
                                     final String title,
    ...
    builder.setSpan(new ForegroundColorSpan(secondaryTextColor), start,
                    builder.length(), 0);
