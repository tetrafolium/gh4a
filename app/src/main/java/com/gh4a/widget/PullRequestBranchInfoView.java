...
public class PullRequestBranchInfoView
  ...
  public void bind(final PullRequestMarker sourceMarker,
                   final PullRequestMarker targetMarker,
                   final GitReference sourceReference) {
    mSourceMarker = sourceMarker;
    mTargetMarker = targetMarker;
    formatMarkerText(mSourceBranchView, R.string.pull_request_from,
                     sourceMarker, sourceReference != null);
    formatMarkerText(mTargetBranchView, R.string.pull_request_to, targetMarker,
                     true);
  }

  private void formatMarkerText(final StyleableTextView view,
                                final @StringRes int formatResId,
                                final PullRequestMarker marker,
                                final boolean makeClickable) {
    ...
    if (pos >= 0) {
      String label =
          TextUtils.isEmpty(marker.label()) ? marker.ref() : marker.label();
      builder.replace(pos, pos + 5, label);
      if (marker.repo() != null && makeClickable) {
        builder.setSpan(new ForegroundColorSpan(mAccentColor), pos,
                        pos + label.length(), 0);
  ...
  public void onClick(final View v) {
    PullRequestMarker marker =
        v.getId() == R.id.tv_pr_from ? mSourceMarker : mTargetMarker;
    if (marker.repo() != null) {
