...
public class CommitStatusBox
  ...
  public CommitStatusBox(final Context context) { this(context, null); }
  ...
  public void fillStatus(final List<Status> statuses,
                         final PullRequest.MergeableState mergableState) {
    ...
    int statusIconResId =
        UiUtils.resolveDrawable(getContext(), statusIconDrawableAttrId);
    ...
    for (Status status : statuses) {
      View statusRow = mInflater.inflate(R.layout.row_commit_status,
                                         mStatusContainer, false);
      ...
      icon.setImageResource(
          UiUtils.resolveDrawable(getContext(), iconDrawableAttrId));
    ...
    if (mergableState == PullRequest.MergeableState.Unstable &&
        pendingCount > 0) {
      int resId = UiUtils.resolveDrawable(getContext(),
                                          R.attr.pullRequestMergeUnknownIcon);
  ...
  private void setSummaryText(final int failingCount, final int pendingCount,
                              final int successCount) {
    ArrayList<String> summaryList = new ArrayList<>();
    if (failingCount > 0) {
      summaryList.add(
          getContext().getString(R.string.check_failing, failingCount));
    }
    if (pendingCount > 0) {
      summaryList.add(
          getContext().getString(R.string.check_pending, pendingCount));
    }
    if (successCount > 0) {
      summaryList.add(
          getContext().getString(R.string.check_successful, successCount));
    }
    String summary = TextUtils.join(", ", summaryList);

    int sumCount = pendingCount + failingCount + successCount;
    mSummaryTextView.setText(getResources().getQuantityString(
        R.plurals.checks_summary, sumCount, summary));
  }

  private void setStatusesExpanded(final boolean expanded) {
    mStatusContainer.setVisibility(expanded ? View.VISIBLE : View.GONE);
    int drawableAttr =
        expanded ? R.attr.dropUpArrowIcon : R.attr.dropDownArrowIcon;
