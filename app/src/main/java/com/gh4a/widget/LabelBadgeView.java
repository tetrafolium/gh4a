...
public class LabelBadgeView extends View {
  ...
  public LabelBadgeView(final Context context, final AttributeSet attrs,
    ...
    TypedArray a = context.obtainStyledAttributes(
        attrs, R.styleable.LabelBadgeView, defStyle, 0);
    mBadgeSize = a.getDimensionPixelSize(
        R.styleable.LabelBadgeView_badgeSize,
        getResources().getDimensionPixelSize(R.dimen.default_label_badge_size));
    mBadgeSpacing =
        a.getDimensionPixelSize(R.styleable.LabelBadgeView_badgeSpacing,
                                getResources().getDimensionPixelSize(
                                    R.dimen.default_label_badge_spacing));
  ...
  protected void onMeasure(final int widthMeasureSpec,
    ...
    mBadgesPerRow = Math.max(
        1, Math.min(getBadgeCount(), maxWidth / (mBadgeSize + mBadgeSpacing)));
    int rows = getBadgeRows();

    int desiredWidth = mBadgesPerRow * mBadgeSize +
                       (mBadgesPerRow - 1) * mBadgeSpacing + getPaddingLeft() +
                       getPaddingRight();
  ...
  private int getBadgeCount() { return mColors != null ? mColors.length : 0; }
