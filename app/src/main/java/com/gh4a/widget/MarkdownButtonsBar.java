...
public class MarkdownButtonsBar
  ...
  private void initialize() {
    ...
    // Container uses different View classes so it can't use the same id or
    // we'll get class cast exception when restoring state.
    mContainer = view.findViewById(R.id.buttons_container);
    if (mContainer == null) {
      mContainer = view.findViewById(R.id.buttons_container_landscape);
    }

    initializeButtons(view, R.id.md_h1, R.id.md_h2, R.id.md_h3, R.id.md_bold,
                      R.id.md_italic, R.id.md_strikethrough,
                      R.id.md_bullet_list, R.id.md_number_list,
                      R.id.md_task_list, R.id.md_divider, R.id.md_code,
                      R.id.md_quote, R.id.md_link, R.id.md_image);
  }
  ...
  public void setEditText(final EditText editText) { mEditText = editText; }

  public void
  setBottomSheetBehavior(final ToggleableBottomSheetBehavior behavior) {
