...
public class PathBreadcrumbs
    extends HorizontalScrollView implements View.OnClickListener {
  ...
  private void init() {
    ...
    mChildFrame.setMinimumHeight(
        (int)getResources().getDimension(R.dimen.breadcrumb_height));
    addView(mChildFrame,
  ...
  private void addCrumb(final @NonNull String path, final String title) {
    ViewGroup view =
        (ViewGroup)mInflater.inflate(R.layout.breadcrumb, mChildFrame, false);
    ...
    if (mChildFrame.getChildCount() > 0) {
      ViewGroup lastChild =
          (ViewGroup)mChildFrame.getChildAt(mChildFrame.getChildCount() - 1);
