...
public class CommitCommentLoadTask extends UrlLoadTask {
  ...
  @VisibleForTesting protected final String mCommitSha;
  @VisibleForTesting protected final IntentUtils.InitialCommentMarker mMarker;

  public CommitCommentLoadTask(final FragmentActivity activity,
  ...
  public static Single<Optional<Intent>>
  load(final Context context, final String repoOwner, final String repoName,
    ...
    Single<Commit> commitSingle =
        commitService.getCommit(repoOwner, repoName, commitSha)
            .map(ApiHelpers::throwOnFailure);
    Single<List<GitComment>> commentSingle =
        ApiHelpers.PageIterator
            .toSingle(page
                      -> commentService.getCommitComments(repoOwner, repoName,
                                                          commitSha, page))
            .cache(); // single is used multiple times -> avoid refetching data

    Single<Optional<GitHubFile>> fileSingle =
        commentSingle
            .compose(RxUtils.filterAndMapToFirst(
                c -> marker.matches(c.id(), c.createdAt())))
    ...
    return Single.zip(
        commitSingle, commentSingle, fileSingle,
        (commit, comments, fileOpt) -> {
          GitHubFile file = fileOpt.orNull();
          if (file != null && !FileUtils.isImage(file.filename())) {
            return Optional.of(CommitDiffViewerActivity.makeIntent(
                context, repoOwner, repoName, commitSha, file.filename(),
                file.patch(), comments, -1, -1, false, marker));
          } else if (file == null) {
            return Optional.of(CommitActivity.makeIntent(
                context, repoOwner, repoName, commitSha, marker));
          }
          return Optional.absent();
        });
  }
