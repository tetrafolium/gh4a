...
public class RefPathDisambiguationTask extends UrlLoadTask {
  ...
  @VisibleForTesting protected final String mRefAndPath;
  @VisibleForTesting protected final int mInitialPage;
  @VisibleForTesting protected final String mFragment;
  @VisibleForTesting protected final boolean mGoToFileViewer;

  public RefPathDisambiguationTask(final FragmentActivity activity,
                                   final String repoOwner,
                                   final String repoName,
                                   final String refAndPath,
  ...
  protected Single<Optional<Intent>> getSingle() {
    return resolve().map(refAndPathOpt -> {
      ...
      if (mGoToFileViewer && refAndPath.second != null) {
        ...
        return Optional.of(FileViewerActivity.makeIntentWithHighlight(
            mActivity, mRepoOwner, mRepoName, refAndPath.first,
            refAndPath.second, highlightStart, highlightEnd));
      } else if (!mGoToFileViewer) {
        return Optional.of(RepositoryActivity.makeIntent(
            mActivity, mRepoOwner, mRepoName, refAndPath.first,
  ...
  private Single<Optional<Pair<String, String>>>
  matchBranch(final Single<List<Branch>> input) {
        ...
        if (TextUtils.equals(mRefAndPath, branch.name())) {
          ...
          if (mRefAndPath.startsWith(nameWithSlash)) {
            return Optional.of(Pair.create(
                branch.name(), mRefAndPath.substring(nameWithSlash.length())));
          }
        }
      }
      return Optional.absent();
  ...
  // returns ref, path
  private Single<Optional<Pair<String, String>>> resolve()
      throws ApiRequestException {
    // first check whether the path redirects to HEAD
    if (mRefAndPath.startsWith("HEAD")) {
      return Single.just(Optional.of(Pair.create(
          "HEAD",
    ...
    return ApiHelpers.PageIterator
        ...
        // and tags after that
        .flatMap(result
                 -> result.orOptionalSingle(
                     ()
                         -> ApiHelpers.PageIterator
                                .toSingle(page
                                          -> repoService.getTags(
                                              mRepoOwner, mRepoName, page))
                                .compose(this::matchBranch)))
        .map(resultOpt -> resultOpt.orOptional(() -> {
          // at this point, the first item may still be a SHA1 - check with a
          // simple regex
          int slashPos = mRefAndPath.indexOf('/');
          String potentialSha =
              slashPos > 0 ? mRefAndPath.substring(0, slashPos) : mRefAndPath;
