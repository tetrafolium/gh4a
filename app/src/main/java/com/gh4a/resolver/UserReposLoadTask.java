...
public class UserReposLoadTask extends UserLoadTask {
  @VisibleForTesting protected final boolean mShowStars;

  public UserReposLoadTask(final FragmentActivity activity,
                           final String userLogin, final boolean showStars) {
  ...
  protected Intent getIntent(final User user) {
    ...
    String filter = mShowStars && !isOrg
                        ...
                        : null;
    return RepositoryListActivity.makeIntent(mActivity, user.login(), isOrg,
                                             filter);
  }
}
