...
public class PullRequestDiffCommentLoadTask extends UrlLoadTask {
  ...
  @VisibleForTesting protected final IntentUtils.InitialCommentMarker mMarker;
  @VisibleForTesting protected final int mPage;

  public PullRequestDiffCommentLoadTask(
      final FragmentActivity activity, final String repoOwner,
      final String repoName, final int pullRequestNumber,
      final IntentUtils.InitialCommentMarker marker, final int page) {
    super(activity);
  ...
  protected Single<Optional<Intent>> getSingle() {
    ...
    Single<PullRequest> pullRequestSingle =
        service.getPullRequest(mRepoOwner, mRepoName, mPullRequestNumber)
            .map(ApiHelpers::throwOnFailure);

    final PullRequestReviewCommentService commentService =
        ServiceFactory.get(PullRequestReviewCommentService.class, false);

    Single<List<ReviewComment>> commentsSingle =
    ...
    Single<List<GitHubFile>> filesSingle = ApiHelpers.PageIterator.toSingle(
        page
    ...
    return commentsSingle
        .compose(RxUtils.filterAndMapToFirst(
            c -> mMarker.matches(c.id(), c.createdAt())))
        ...
        .flatMap(result -> {
          if (result.isPresent()) {
            ...
            if (foundComment && file != null &&
                !FileUtils.isImage(file.filename())) {
              return Single.zip(
                  pullRequestSingle, commentsSingle, (pr, comments) -> {
                    // noinspection CodeBlock2Expr
                    return Optional.of(PullRequestDiffViewerActivity.makeIntent(
                        mActivity, mRepoOwner, mRepoName, mPullRequestNumber,
                        pr.head().sha(), file.filename(), file.patch(),
                        comments, -1, -1, -1, false, mMarker));
                  });
            }
            if (foundComment && file == null) {
              return Single.just(Optional.of(PullRequestActivity.makeIntent(
                  mActivity, mRepoOwner, mRepoName, mPullRequestNumber, mPage,
                  mMarker)));
            }
          }
