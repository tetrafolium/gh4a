...
public class PullRequestReviewCommentLoadTask extends UrlLoadTask {
  ...
  public PullRequestReviewCommentLoadTask(
        ...
        .flatMap(comments -> {
          ...
          for (ReviewComment comment : comments) {
            ...
            if (!commentsByDiffHunkId.containsKey(id)) {
              // Because the comment we are looking for could be a reply to
              // another review we have to keep track of initial comments for
              // each diff hunk
              commentsByDiffHunkId.put(id, comment);
            }

            if (marker.matches(comment.id(), null)) {
              // Once found the comment we are looking for get a correct review
              // id from the initial diff hunk comment
