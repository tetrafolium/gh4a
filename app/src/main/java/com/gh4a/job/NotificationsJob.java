...
public class NotificationsJob extends Job {
  private static final String CHANNEL_GITHUB_NOTIFICATIONS =
      "channel_notifications";
  private static final String GROUP_ID_GITHUB = "github_notifications";
  public static final String TAG = "job_notifications";

  private static final String KEY_LAST_NOTIFICATION_CHECK =
      "last_notification_check";
  private static final String KEY_LAST_NOTIFICATION_SEEN =
      "last_notification_seen";
  private static final String KEY_LAST_SHOWN_REPO_IDS =
  ...
  public static void createNotificationChannels(final Context context) {
    ...
    channel.setDescription(
        context.getString(R.string.channel_notifications_description));
  ...
  public static void handleNotificationDismiss(final Context context,
                                               final int id) {
    ...
    synchronized (sPrefsLock) {
      Set<String> lastShownRepoIds =
          prefs.getStringSet(KEY_LAST_SHOWN_REPO_IDS, null);
  ...
  protected Result onRunJob(final Params params) {
    ...
    synchronized (sPrefsLock) {
      ...
      if (!hasUnseenNotification) {
        ...
        // so everything is canceled at this point and we have nothing to notify
        // of
  ...
  private void
  showRepoNotification(final NotificationManagerCompat nm,
    ...
    String text = getContext().getResources().getQuantityString(
        R.plurals.unread_notifications_summary_text, notifications.size(),
        notifications.size());

    Intent intent =
        NotificationHandlingService.makeOpenNotificationActionIntent(
            getContext(), repository.owner().login(), repository.name());
    PendingIntent contentIntent = PendingIntent.getService(
        getContext(), id, intent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    Intent markReadIntent =
        NotificationHandlingService
            .makeMarkReposNotificationsAsReadActionIntent(
                getContext(), id, repository.owner().login(),
                repository.name());
    PendingIntent markReadPendingIntent = PendingIntent.getService(
        getContext(), id, markReadIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    android.app.Notification publicVersion =
        makeBaseBuilder()
            .setContentTitle(getContext().getString(
                R.string.unread_notifications_summary_title))
            .setContentText(text)
    ...
    NotificationCompat.MessagingStyle style =
        new NotificationCompat.MessagingStyle("").setConversationTitle(title);
    for (int i = notifications.size() - 1; i >= 0; i--) {
      NotificationThread n = notifications.get(i);
      style.addMessage(n.subject().title(), n.updatedAt().getTime(),
                       determineNotificationTypeLabel(n));
      hasNewNotification =
          hasNewNotification || n.updatedAt().getTime() > lastCheck;
    }
  ...
  private void showSummaryNotification(
      ...
      final List<List<NotificationThread>> notificationsPerRepo,
      final boolean hasNewNotification) {
    int totalCount = 0;
    ...
    String title =
        getContext().getString(R.string.unread_notifications_summary_title);
    String text = getContext().getResources().getQuantityString(
    ...
    PendingIntent deleteIntent = PendingIntent.getService(
        getContext(), 0,
        NotificationHandlingService.makeMarkNotificationsSeenIntent(
            getContext()),
        0);
    NotificationCompat.Builder builder =
        makeBaseBuilder()
    ...
    NotificationCompat.InboxStyle inboxStyle =
        new NotificationCompat.InboxStyle(builder).setBigContentTitle(text);
    for (List<NotificationThread> list : notificationsPerRepo) {
      Repository repository = list.get(0).repository();
      String repoName = repository.owner().login() + "/" + repository.name();
      final TextAppearanceSpan notificationPrimarySpan = new TextAppearanceSpan(
          getContext(), R.style.TextAppearance_NotificationEmphasized);
      final int emphasisEnd;

      SpannableStringBuilder line =
          new SpannableStringBuilder(repoName).append(" ");
      if (list.size() == 1) {
        ...
        emphasisEnd = line.length();
        line.append(getContext().getResources().getQuantityString(
            R.plurals.notification, list.size(), list.size()));
      }
  ...
  private NotificationCompat.Builder makeBaseBuilder() {
    return new NotificationCompat
        .Builder(getContext(), CHANNEL_GITHUB_NOTIFICATIONS)
  ...
  private Bitmap loadRoundUserAvatar(final User user) {
    Bitmap avatar =
        AvatarHandler.loadUserAvatarSynchronously(getContext(), user);
    if (avatar == null) {
      return null;
    }

    final Bitmap output = Bitmap.createBitmap(
        avatar.getWidth(), avatar.getHeight(), Bitmap.Config.ARGB_8888);
