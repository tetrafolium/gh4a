...
public abstract class TimelineItem {
  ...
  public static class TimelineComment extends TimelineItem {
    ...
    @NonNull private GitHubCommentBase comment;

    @Nullable public final GitHubFile file;
    ...
    public TimelineComment(final @NonNull ReviewComment comment,
                           final @Nullable GitHubFile file) {
      this.comment = comment;
      this.file = file;
    }

    public GitHubCommentBase comment() { return comment; }
    ...
    public void setReactions(final Reactions reactions) {
      if (comment instanceof ReviewComment) {
        comment =
            ((ReviewComment)comment).toBuilder().reactions(reactions).build();
      } else {
        comment =
            ((GitHubComment)comment).toBuilder().reactions(reactions).build();
      }
    }
    ...
    public Diff getParentDiff() { return diff; }
    ...
    @Nullable
    public Intent makeDiffIntent(final Context context, final int line,
      ...
      Matcher matcher =
          PULL_REQUEST_PATTERN.matcher(reviewComment.pullRequestUrl());
      if (matcher.matches()) {
        ...
        IntentUtils.InitialCommentMarker initialComment =
            line == -1
                ? new IntentUtils.InitialCommentMarker(reviewComment.id())
                : null;

        return PullRequestDiffViewerActivity.makeIntent(
            context, repoOwner, repoName, pullRequestNumber,
            reviewComment.commitId(), reviewComment.path(), file.patch(), null,
            -1, line, line, isRightNumber, initialComment);
      }
  ...
  public static class TimelineEvent extends TimelineItem {
    @NonNull public final IssueEvent event;
  ...
  public static class TimelineReview extends TimelineItem {
    @NonNull private final Review review;
    ...
    /**
     * Add the specified comment and its matching commit file to correct diff
     * hunk inside of this review. NOTE: This method expects to be called first
     * with comments from this review in a sorted order.
     *
     * @param comment The comment to add to this review.
     * @param file The commit file in which the comment was created.
     * @param addNewDiffHunk {@code true} if new diff hunk should be created if
     *     it's not found.
     */
    public void addComment(final @NonNull ReviewComment comment,
                           final @Nullable GitHubFile file,
                           final boolean addNewDiffHunk) {
      // Comments are grouped by a special diff hunk id which is a combination
      // of these 3 fields. By using this id we can display comments and their
      // replies together under a single diff hunk. NOTE: Using this id is not
      // correct in all of the possible cases (Comments created with "Start new
      // conversation" are incorrect). Sadly the GitHub API doesn't provide
      ...
      if (diffHunk == null) {
        ...
        if (diffHunk.isReply()) {
          // To match how GitHub web interface works diff hunks with this
          // property set to true should display only the initial comments.
          return;
        }

        if (diffHunk.getCreatedAt() != null &&
            diffHunk.getCreatedAt().after(comment.createdAt())) {
          // Because we are adding all of the comments in order based on their
          // creation date and also first add only comments from the initial
          // review we know that if something comes out of order then our
          // initial comment was a reply.
  ...
  public static class Diff extends TimelineItem implements Comparable<Diff> {
    @NonNull public final List<TimelineComment> comments = new ArrayList<>();

    private boolean mIsReply;

    /**
     * Returns special id that is used to group comments together with all
     * replies to them under the same diff hunk.
     *
    ...
    /**
     * Indicates whether this diff hunk corresponds to an initial review comment
     * which was created as a reply to different review comment. <p> This
     * property is used to match how GitHub web interface works. These comments
     * are special because for them reply button is not displayed and they are
     * the only comments that should be visible in the corresponding diff hunk.
     */
    public boolean isReply() { return mIsReply; }

    /**
     * Mark that this diff hunk corresponds to an initial review comment that
     * was created as reply to different review comment.
     */
    public void setIsReply(final boolean isReply) { mIsReply = isReply; }
    ...
    public int compareTo(final @NonNull Diff other) {
      ...
      // Null created date means that the diff is for a pending review, place
      // these after other diffs.
      if (createdAt == null) {
        return 1;
      }
  ...
  public static class Reply extends TimelineItem {
    @NonNull public final TimelineComment timelineComment;
  ...
  @Nullable public abstract User getUser();
