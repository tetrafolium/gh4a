...
public class TrendAdapter extends RootAdapter<Trend, TrendAdapter.ViewHolder> {
  ...
  public TrendAdapter(final Context context,
                      final @StringRes int starsTemplate) {
    super(context);
  ...
  public void onBindViewHolder(final ViewHolder holder, final Trend trend) {
    ...
    holder.tvDesc.setText(desc != null ? EmojiParser.parseToUnicode(desc)
                                       : null);
    ...
    holder.tvStars.setText(mContext.getString(
        mStarsTemplate, trend.getNewStars(), trend.getStars()));
