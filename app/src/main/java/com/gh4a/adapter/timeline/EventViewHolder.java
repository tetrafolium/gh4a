...
class EventViewHolder
    extends TimelineItemAdapter
                .TimelineItemViewHolder<TimelineItem.TimelineEvent>
    implements View.OnClickListener {

  private static final Pattern COMMIT_URL_REPO_NAME_AND_OWNER_PATTERN =
      Pattern.compile(".*github\\.com/repos/([^/]+)/([^/]+)/commits");
  private static final HashMap<IssueEventType, Integer> EVENT_ICONS =
      new HashMap<>();
  ...
  static {
    ...
    EVENT_ICONS.put(IssueEventType.Demilestoned,
                    R.attr.issueEventDemilestonedIcon);
    EVENT_ICONS.put(IssueEventType.Renamed, R.attr.issueEventRenamedIcon);
    EVENT_ICONS.put(IssueEventType.HeadRefDeleted,
                    R.attr.timelineEventBranchIcon);
    EVENT_ICONS.put(IssueEventType.HeadRefRestored,
                    R.attr.timelineEventBranchIcon);
    EVENT_ICONS.put(IssueEventType.ReviewRequested,
                    R.attr.timelineEventReviewRequested);
    EVENT_ICONS.put(IssueEventType.ReviewRequestRemoved,
                    R.attr.timelineEventReviewRequestRemoved);
  }
  ...
  public EventViewHolder(final View itemView, final String repoOwner,
                         final String repoName, final boolean isPullRequest) {
    super(itemView);
  ...
  public void bind(final TimelineItem.TimelineEvent item) {
    User user = item.event.assigner() != null ? item.event.assigner()
                                              : item.event.actor();
    ...
    if (eventIconAttr != null) {
      mEventIconView.setImageResource(
    ...
    mMessageView.setText(formatEvent(
        item.event, user, mMessageView.getTypefaceValue(), mIsPullRequest));
  }
  ...
  private CharSequence formatEvent(final IssueEvent event, final User user,
    ...
    case Closed:
      ...
      break;
    case Reopened:
      textResId = isPullRequestEvent ? R.string.pull_request_event_reopened
    ...
    case Unassigned: {
      ...
      String assigneeLogin =
          event.assignee() != null ? event.assignee().login() : null;
      if (assigneeLogin != null && assigneeLogin.equals(actorLogin)) {
        ...
        textResId = isAssign ? R.string.issue_event_assigned
                             : R.string.issue_event_unassigned;
        textBase = mContext.getString(
            textResId, ApiHelpers.getUserLogin(mContext, user),
    ...
    case Demilestoned:
      ...
      textBase =
          mContext.getString(textResId, ApiHelpers.getUserLogin(mContext, user),
    ...
    case ReviewRequestRemoved: {
      ...
      if (event.requestedReviewers() != null) {
        ...
        reviewerNames =
            ApiHelpers.getUserLogin(mContext, event.requestedReviewer());
      }
      @StringRes
      int stringResId =
          event.event() == IssueEventType.ReviewRequested
              ? R.string.pull_request_event_review_requested
              : R.string.pull_request_event_review_request_removed;
      textBase = mContext.getString(
          stringResId,
          ApiHelpers.getUserLogin(mContext, event.reviewRequester()),
          reviewerNames);
      break;
    }
    case HeadRefDeleted:
    ...
    SpannableStringBuilder text =
        StringUtils.applyBoldTags(textBase, typefaceValue);
    ...
    if (event.commitId() != null && pos >= 0) {
      ...
      text.setSpan(new IntentSpan(mContext, context -> {
                     // The commit might be in a different repo. The API doesn't
                     // provide that information directly, so get it indirectly
                     // by parsing the URL
                     ...
                     if (url != null) {
                       Matcher matcher =
                           COMMIT_URL_REPO_NAME_AND_OWNER_PATTERN.matcher(url);
                       if (matcher.find()) {
                     ...
                     return CommitActivity.makeIntent(
                         context, repoOwner, repoName, event.commitId());
                   }), pos, pos + 7, 0);
    ...
    if (label != null && pos >= 0) {
      ...
      text.setSpan(new IssueLabelSpan(mContext, label, false), pos,
                   pos + length, 0);
    }
