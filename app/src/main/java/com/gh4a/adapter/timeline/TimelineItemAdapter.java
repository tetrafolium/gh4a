...
public class TimelineItemAdapter
    extends RootAdapter<TimelineItem,
                        TimelineItemAdapter.TimelineItemViewHolder>
  ...
  private final CommentViewHolder
      .Callback mCommentCallback = new CommentViewHolder.Callback() {
    @Override
    public boolean canQuote() {
      return !mLocked;
    }
    ...
    @Override
    public boolean onMenItemClick(final TimelineItem.TimelineComment comment,
    ...
    @Override
    public Single<List<Reaction>> loadReactionDetails(
        final TimelineItem.TimelineComment item, final boolean bypassCache) {
      return mActionCallback.loadReactionDetails(item.comment(), bypassCache);
    }

    @Override
    public Single<Reaction> addReaction(final TimelineItem.TimelineComment item,
  ...
  public TimelineItemAdapter(final Context context, final String repoOwner,
                             final String repoName, final int issueNumber,
                             final boolean isPullRequest,
                             final boolean displayReviewDetails,
                             final OnCommentAction callback) {
    super(context);
  ...
  public void suppressCacheClearOnNextClear() { mDontClearCacheOnClear = true; }
  ...
  onCreateViewHolder(final LayoutInflater inflater, final ViewGroup parent,
                     final int viewType) {
    View view;
    ...
    case VIEW_TYPE_REVIEW:
      ...
      holder = new ReviewViewHolder(view, mImageGetter, mRepoOwner, mRepoName,
                                    mIssueNumber, mDisplayReviewDetails,
                                    mReviewCallback);
      break;
    case VIEW_TYPE_DIFF:
    ...
    default:
      throw new IllegalArgumentException(
          "viewType: Unknown timeline item type.");
    }
    return holder;
  }
  ...
  @Override
  public void onBindViewHolder(final TimelineItemViewHolder holder,
  ...
  private boolean shouldFadeReplyGroup(final TimelineItem item) {
    ...
    if (item instanceof TimelineItem.Diff) {
      return ((TimelineItem.Diff)item).getInitialComment().id() !=
          replyCommentId;
    }
    if (item instanceof TimelineItem.TimelineComment) {
  ...
  public static abstract class TimelineItemViewHolder<TItem
