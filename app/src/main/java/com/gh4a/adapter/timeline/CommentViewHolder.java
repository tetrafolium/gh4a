...
class CommentViewHolder
    extends TimelineItemAdapter
                .TimelineItemViewHolder<TimelineItem.TimelineComment>
  ...
  public interface Callback {
    ...
    boolean onMenItemClick(TimelineItem.TimelineComment comment,
                           MenuItem menuItem);
    Single<List<Reaction>>
    loadReactionDetails(TimelineItem.TimelineComment item, boolean bypassCache);
    Single<Reaction> addReaction(TimelineItem.TimelineComment item,
                                 String content);
  }

  public CommentViewHolder(
      final View view, final HttpImageGetter imageGetter,
    ...
    mReactionMenuHelper = new ReactionBar.AddReactionMenuHelper(
        view.getContext(), reactItem.getSubMenu(), this, this,
  ...
  public void bind(final TimelineItem.TimelineComment item) {
    ...
    if (createdAt.equals(updatedAt) || item.getReviewComment() != null) {
      // Unlike issue comments, the update timestamp for commit comments also
      // changes when e.g. the line number changes due to the diff the comment
      // was made on becoming outdated. As we can't distinguish those updates
      // from comment body updates, hide the edit timestamp for all commit
      // comments.
      tvEditTimestamp.setVisibility(View.GONE);
    } else {
      tvEditTimestamp.setText(
    ...
    if (association != null) {
      ...
      userName.setSpan(new RelativeSizeSpan(0.85f), start, userName.length(),
                       0);
      int color =
          UiUtils.resolveColor(mContext, android.R.attr.textColorSecondary);
      userName.setSpan(new ForegroundColorSpan(color), start, userName.length(),
                       0);
    }
    ...
    int position = item.getReviewComment() != null &&
                           item.getReviewComment().position() != null
                       ? item.getReviewComment().position()
                       : -1;
    ...
    menu.findItem(R.id.view_in_file)
        .setVisible(item.file != null && position != -1);
  }
  ...
  public boolean onMenuItemClick(final MenuItem menuItem) {
    TimelineItem.TimelineComment comment =
        (TimelineItem.TimelineComment)ivMenu.getTag();
