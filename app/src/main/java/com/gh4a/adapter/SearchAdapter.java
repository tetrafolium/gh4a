...
public class SearchAdapter
    extends RootAdapter<Object, RecyclerView.ViewHolder> {
  ...
  public void onBindViewHolder(final RecyclerView.ViewHolder holder,
    ...
    if (item instanceof User) {
      ...
      mRepoAdapter.onBindViewHolder((RepositoryAdapter.ViewHolder)holder,
                                    (Repository)item);
    } else {
      mCodeAdapter.onBindViewHolder((CodeSearchAdapter.ViewHolder)holder,
                                    (SearchCode)item);
    }
  }

  public static class CodeSearchAdapter
      extends RootAdapter<SearchCode, CodeSearchAdapter.ViewHolder> {
      ...
      if (matches != null && !matches.isEmpty()) {
        ...
        for (int i = 0; i < matches.size(); i++) {
          ...
          SpannableStringBuilder builder =
              new SpannableStringBuilder(match.fragment());
        ...
        for (int i = matches.size();
             i < holder.matchesContainer.getChildCount(); i++) {
    ...
    public void onClick(final View view) {
      if (view.getId() == R.id.tv_match) {
        ...
        mCallback.onSearchFragmentClick(
            searchResult, (int)view.getTag(R.id.search_match_index));
        return;
      }
