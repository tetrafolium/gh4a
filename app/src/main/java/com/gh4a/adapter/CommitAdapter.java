...
public class CommitAdapter
    extends RootAdapter<Commit, CommitAdapter.ViewHolder> {
  public CommitAdapter(final Context context) { super(context); }
  ...
  public void onBindViewHolder(final ViewHolder holder, final Commit commit) {
    ...
    if (author != null && !TextUtils.isEmpty(author.login())) {
      ...
      holder.ivGravatar.setImageDrawable(
          new AvatarHandler.DefaultAvatarDrawable(userName, email));
    }
    ...
    holder.ivDescriptionIndicator.setVisibility(pos > 0 ? View.VISIBLE
                                                        : View.GONE);
    ...
    holder.tvTimestamp.setText(StringUtils.formatRelativeTime(
        mContext, commit.commit().author().date(), false));
  }
  ...
  public void onClick(final View v) {
    if (v.getId() == R.id.iv_gravatar) {
      ...
      Intent intent =
          UserActivity.makeIntent(mContext, ApiHelpers.getAuthorLogin(commit));
      if (intent != null) {
