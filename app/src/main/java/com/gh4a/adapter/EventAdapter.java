...
public class EventAdapter
    extends RootAdapter<GitHubEvent, EventAdapter.EventViewHolder> {
  public EventAdapter(final Context context) { super(context); }

  @Override
  public EventViewHolder onCreateViewHolder(final LayoutInflater inflater,
  ...
  @Override
  public void onBindViewHolder(final EventViewHolder holder,
    ...
    holder.tvCreatedAt.setText(
        StringUtils.formatRelativeTime(mContext, event.createdAt(), false));

    CharSequence content =
        formatDescription(event, holder.tvDesc.getTypefaceValue());
  ...
  private CharSequence formatDescription(final GitHubEvent event,
    ...
    case CreateEvent: {
      ...
      if (payload.refType() == ReferenceType.Repository) {
        return res.getString(R.string.event_create_repo_desc,
                             eventRepo.repoWithUserName());
      }
      break;
    }
    ...
    case FollowEvent: {
      ...
      if (target != null) {
        return res.getString(R.string.event_follow_desc, target.login(),
                             target.publicRepos(), target.followers());
      }
      break;
    }
    ...
    case ForkEvent: {
      ...
      return res.getString(R.string.event_fork_desc,
                           formatToRepoName(payload.forkee()));
    }
    ...
    case GollumEvent: {
      ...
      if (pages != null && !pages.isEmpty()) {
        return res.getString(R.string.event_gollum_desc,
                             pages.get(0).pageName());
      }
      break;
    }
    ...
    case PushEvent: {
      ...
      if (commits != null && !commits.isEmpty()) {
        ...
        int maxLines = mContext.getResources().getInteger(
            R.integer.event_description_max_lines);
        int max =
            Math.min(count > maxLines ? maxLines - 1 : count, commits.size());
        ...
        for (int i = 0; i < max; i++) {
          ...
          ssb.append(sha);
          ssb.setSpan(
              new TextAppearanceSpan(mContext, R.style.small_highlighted_sha),
              ssb.length() - sha.length(), ssb.length(), 0);

          ssb.append(" ");
          ssb.append(
              getFirstLine(EmojiParser.parseToUnicode(commit.message())));
          ssb.setSpan(
              new EllipsizeLineSpan(i == (count - 1) ? 0 : bottomMargin),
  ...
  private String formatTitle(final GitHubEvent event) {
    ...
    case GistEvent: {
      ...
      String id =
          gist != null ? gist.id() : mContext.getString(R.string.deleted);
      int resId = payload.action() == GistPayload.Action.Updated
                      ? R.string.event_update_gist_title
                      : R.string.event_create_gist_title;
    ...
    case PullRequestEvent: {
      ...
      case Opened:
        ...
        break;
      case Closed:
        resId = pr.merged() ? R.string.event_pr_merge_title
      ...
      return res.getString(resId, payload.number(),
                           formatFromRepoIdentifier(eventRepo));
    }
  ...
  private String
  formatFromRepoIdentifier(final GitHubEvent.RepoIdentifier repository) {
    if (repository != null) {
