...
public class CommonFeedAdapter
    extends RootAdapter<Feed, CommonFeedAdapter.ViewHolder> {
  ...
  public void onBindViewHolder(final ViewHolder holder, final Feed feed) {
    ...
    holder.tvDesc.setGravity(mShowExtra ? Gravity.TOP
                                        : Gravity.CENTER_VERTICAL);

    if (mShowExtra && feed.getUserId() > 0) {
      AvatarHandler.assignAvatar(holder.ivGravatar, feed.getAuthor(),
    ...
    if (mShowExtra) {
      Date date =
          feed.getPublished() != null ? feed.getPublished() : feed.getUpdated();
      String published =
          date != null ? DateFormat.getMediumDateFormat(mContext).format(date)
                       : "";
      holder.tvExtra.setText(feed.getAuthor());
      holder.tvTimestamp.setText(published);
      holder.tvExtra.setVisibility(feed.getAuthor() != null ? View.VISIBLE
                                                            : View.GONE);
